...
proc vTcl:check_upon_close_button {base} {
    # We check to see if we are closing the Python Console contain a
    # modified support module
    global vTcl
    set exists [winfo exists $vTcl(code_window)]
    if {!$exists} return
    if {[$vTcl(code_window) edit modified]} {
        set reply [tk_dialog .foo "Save Question" \
                       "Do you wish to save the content of the Python Console" \
...
proc vTcl:Window.vTcl.py_console {} {
    ...
    wm geometry $top 800x$start_h+400+50; update
...
#        -background $vTcl(pr,bgcolor) ;# 11/22/12
    ...
    button  $top.butframe.but35 \
        -text Close -command "vTcl:check_upon_close_button $base" \
        -background $vTcl(pr,bgcolor)  -foreground $vTcl(pr,fgcolor) ;# 11/22/12
...
-anchor se
    ...
    bind $base <Control-w> {vTcl:check_upon_close_button .vTcl.py_console}
