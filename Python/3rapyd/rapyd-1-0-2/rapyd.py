...
VersionNumber        = '1.0.2'
RpjFileVersionNumber = 1
CopyrightDate        = '2005-2014'
...
class MenuAids:
    ...
    def GenerateBindings(self,BindTargetName):
        ...
        for Action,EventString in Scheme['Actions']:
            ...
            ##D('%s: binding: %s to self.on_Action%s'%(BindTargetName,EventString,Action))
            try:
                exec(Temp)
...
def APD():
    """
    """
    assert Cfg.Info['ProjectDir'] == '' or Cfg.Info['ProjectDir'][-1] == os.path.sep
...
def DictPretty(SomeDir):
    ...
    Result = ['{']
    for Key in Keys:
        Result.append('%s: %s'%(strx(Key),strx(SomeDir[Key])))
    Result.append('}')
    return '\n'.join(Result)
...
def GenerateImportLines(ImportModuleList):
    ...
    for IM in ImportModuleList:
        ...
        if Type == 'from':
            ...
            raise Exception, 'Unknown import type "%s" for module "%s"'%(strx(Type),IM)    
    Result.append('')
    Result = '\n'.join(Result)
    return Result
...
def DiffFind(A,B,History=''):
    ...
    if type(A) in (type([]),type(())):
        ...
        if Akeys <> Bkeys:
            ...
            print 'A_keys: %s'%strx(Akeys)
            print 'B_keys: %s'%strx(Bkeys)
            return
        for Key in A.keys():
...
def strx(Arg):
    """
    This is like str() but if str() fails it will use unicode() instead.
    """
    try:
        Result = str(Arg)
...
def HandlerNameDecompose(HandlerName,WidgetName,Revise=False,NewWidgetName=''):
    """=u
    ...
    This routine checks the passed handler name and returns a non-Null result if:
        
    o The HandlerName is of the form A_B_C, and
    o The A part is "'on'", and
    o The B part matches the passed WidgetName (see note).

    Otherwise we return None.

    Note: The matching is done irrespective of single or double underscores at the start of
        the Handler and Widget names

    If Revise is False then if the names match we return the event string abbreviation of the event handler name.

    If Revise is True then if the names match we return a handler name which has been revised to match
        the NewWidgetName.
    """
    #Strip off and ignore possible prefix underscores
    PrefixH, HandlerName = Rpo.WidgetNameSplit(HandlerName)
    PrefixW, WidgetName = Rpo.WidgetNameSplit(WidgetName)
    ...
    if Temp[0] == 'on' and Temp[1] == WidgetName:
        if Revise:
            #Caller wants a revised handler name
            PrefixWN, NewWidgetName = Rpo.WidgetNameSplit(NewWidgetName)
            Result = '%son_%s_%s'%(PrefixWN, NewWidgetName, Temp[2])
        else:
            #Caller wants the event string abbreviation
            Result = Temp[2]
    return Result
...
def BlankToNone(Arg):
    ...
    """
    if type(Arg) in (type(''),type(u'')) and Arg.strip() == '':
        return None
    return Arg    
...
class BindMixin:
    ...
    def HandlerRename(self,Old,New):
        ...
        TextEditor = GblLayout.TextEditFetch()
        TextWidget = TextEditor.TextWidget
        Temp = TextWidget.ChunkCut(I)
        ...
        #Tell the text editor about the rename; it needs this for undo to work properly
        TextEditor.HandlerRenameNotify(Old,New)
    ...
    def HandlerEnsure(self,HandlerName,HandlerText=None):
        ...
        if Index == '':
            ...
            TextEditor.insert(From,Usertext,'',PowerPill=1)
            TextEditor.insert(From,Systext,'bgldt',PowerPill=1)
    ...
    def HandlerDelete(self,HandlerName):
        ...
        #Have the current option/bind/pack editor refresh itself if needed.                
        GblLayout.TheWidgetator.EditorRefresh()                
        #Tell the text editor about the delete; it needs this for undo to work properly
        GblLayout.TextEditFetch().HandlerDeleteNotify(HandlerName)
        ...
        if NewName == '*DELETE*':
            ...
            if Count == 1:
                ...
                if Choice == 1:
                    ...
                    assert SystextIndex <> '','We were sort of expecting to find the handler'
                    Temp = TextEditor.ChunkCut(SystextIndex)
                    #Tell the text editor about the delete, it needs it for undo
                    TextEditor.HandlerDeleteNotify(OriginalName)
            return 0
        else:        
...
class TextEdit(Frame, BindMixin, MenuAids):
    ...
    def __init__(self,Master,**kw):
        ...
        self.ActionMenuAdd('Undo')
        self.ActionMenuAdd('Redo')
        self.ActionMenuSep()
        ...
        if Cfg.Info['Debug']:
            ...
            self.DebugMenu.add_command(label='Dump the Undo stack',command=Wow(self,'on_Debug','DumpUndo'))
        ...
        self.TextWidget.bind('<ButtonRelease-1>',self.on_TextWidgetRelease,add='+')
    ...
    def HandlerGoto(self,HandlerName):
        ...
        """
        self.TextWidget.HandlerGoto(HandlerName)
        self.StatusUpdate()
        return
        
        Target = self.TextWidget.search('def %s('%HandlerName,'1.0')
        ##D('HandlerGoto: HandlerName="%s", Target="%s"'%(HandlerName, Target))
        if Target <> '':
            self.TextWidget.mark_set(INSERT,Target)
    ...
    def HandlerDeleteNotify(self,HandlerName):
        """
        Note that a handler has been deleted
        """
        self.TextWidget.HandlerDeleteNotify(HandlerName)
        for J in reversed(range(len(self.LineStack))):
            Base,Offset = self.LineStack[J].split('+')
            if Base == HandlerName:
                #Handler has been deleted
                del self.LineStack[J]
        self.StatusUpdate()

    def HandlerRenameNotify(self,Old,New):
        """
        Note that the name of a handler has changed.
        """
        self.TextWidget.HandlerRenameNotify(Old,New)
        #Update LineStack as needed.
        for J in range(len(self.LineStack)):
            Base,Offset = self.LineStack[J].split('+')
            if Base == Old:
                #Handler has been renamed
                self.LineStack[J] = '%s+%s'%(New,Offset)
        self.StatusUpdate()
    ...
    def StatusUpdate(self):
        ...
        if self.LineStack:
            #Format just the line numbers for the user to see
            Lines = []
            for LS in self.LineStack:
                LSD = self.TextWidget.UndoDecode(LS)
                ##D('StatusUpdate decoded %s to %s'%(LS,LSD))
                if LSD <> None:
                    Lines.append(LSD.split('.')[0])
            Lines = ', '.join(reversed(Lines))
            #LS = ', '.join(list(reversed([str(X[0]) for X in self.LineStack])))
        else:
            Lines = '<none>'
        self.InfoLabel['text'] = 'Line: %s of %s.   Pushed line numbers: %s.'%(int(Y),Size,Lines)
    ...
    def on_ActionDeleteLine(self,Event=None):
        ...
        if self.TextWidget.SystextInRegion(From,To):
            #You can't delete generated text
    ...
    def on_ActionIndentBlock(self,Event=None):
        ...
        Current, Last = Extent
        while Current < Last:
            self.TextWidget.insert('%s.0'%Current,Pad,'',UndoSeparator=False)
            Current += 1
        #Placing a single separator now will make an undo atomic
        self.TextWidget.UndoSeparator()
        return 'break'    
    ...
    def on_ActionOutdent(self,Event=None):
        ...
        for J in range(Count):
            ...
            if ' ' <> T:
                ...
                break
            #Delete the space.
            self.TextWidget.delete(I,UndoSeparator=False)
        #Placing a single separator now will make an undo atomic
        self.TextWidget.UndoSeparator()
        return 'break'
    ...
    def on_ActionOutdentBlock(self,Event=None):
        ...
        Current, Last = Extent
        while Current < Last:
            self.TextWidget.delete('%s.0'%Current,'%s.%s'%(Current,Len),UndoSeparator=False)
            Current += 1
        #Placing a single separator now will make an undo atomic
        self.TextWidget.UndoSeparator()
        return 'break'
    ...
    def on_ActionPaste(self,Event=None):
        ...
        try:
            ...
            #We belay the undo separator here and let the mark_set, a few lines below, generate the
            #    separator. That way they both undo at once which is what we want.
            self.TextWidget.insert(INSERT, text=Temp, tags='', UndoSeparator=False)
    ...
    def on_ActionPasteFile(self,Event=None):
        ...
        #We belay the undo separator here and let the mark_set, a few lines below, generate the
        #    separator. That way they both undo at once which is what we want.
        self.TextWidget.insert(INSERT,Text,tags='',ProgressFunction=ProgressFunc,UndoSeparator=False)
        File.close()
    ...
    def on_ActionPopToLineNumber(self,Event=None):
        ...
        """
        #Get rid of any zombie references
        self.LineStackNormalize()
        #Now do the pop
        if self.LineStack:
            R = self.LineStack.pop()
            R = self.TextWidget.UndoDecode(R)
            assert R <> None
            self.TextWidget.mark_set(INSERT,R)
            self.TextWidget.see(INSERT)
            self.TextWidget.ColorizeVisible()
            self.StatusUpdate()
        else:
    ...
    def on_ActionPushLineNumber(self,Event=None):
        ...
        """
        self.LineStack.append(self.TextWidget.UndoEncode(self.TextWidget.index(INSERT)))
        self.StatusUpdate()
        return 'break'    
    ...
    def on_ActionRedo(self,Event=None):
        """
        Redo
        """
        if not self.TextWidget.Redo():
            Rpw.MessageDialog(Message='Nothing to redo')
    ...
    def on_ActionSearchReplace(self,Event=None):        
        ...
        while 1:
            ...
            if Where:
                ...
                #    before the text if searching backward. We create an undo-entryr only if we
                #    are going to do the replace.
                if Info['Back']:
                    self.TextWidget.mark_set(INSERT,Where,Undo=Doit)
                else:
                    self.TextWidget.mark_set(INSERT,Past,Undo=Doit)
                if Doit:
                    ReplaceCount += 1
                    #We belay undo separators here so the undo is atomic. Mark_set above generated the needed sep.
                    self.TextWidget.delete(Where,Past,UndoSeparator=False)
                    self.TextWidget.insert(Where,Info['RepText'],'',UndoSeparator=False)
                    if not Info['All']:
                        self.TextWidget.see(INSERT)
                        return 'break'
            else:
    ...
    def on_ActionSwapWithTop(self,Event=None):
        ...
        if self.LineStack:
            R = self.LineStack[-1]
            self.LineStack[-1] = self.TextWidget.UndoEncode(self.TextWidget.index(INSERT))
            self.TextWidget.mark_set(INSERT,self.TextWidget.UndoDecode(R))
            self.TextWidget.see(INSERT)
            self.TextWidget.ColorizeVisible()
            self.StatusUpdate()
        else:
    ...
    def on_ActionUndo(self,Event=None):
        """
        Undo
        """
        if not self.TextWidget.Undo():
            Rpw.MessageDialog(Message='Nothing to undo')
    ...
    def LineStackNormalize(self):
        """
        Delete any LineStack entried that point to a now-deleted event handler.
        """
        for I in reversed(range(len(self.LineStack))):
            T = self.TextWidget.UndoDecode(self.LineStack[I])
            if T == None:
                #The target of that reference is gone
                del self.LineStack[I]
    ...
    def on_KeyNext(self,Event=None):
        ...
        TextWidgets have built-in PgUp/PgDn handling, but I don't much like the way
            they work, thus we roll our own here.
        
        The messing about computing Delta, the number of lines to jump, is in aid of
            moving as far as we can without skipping over any lines. If the cursor is
            in the middle of the window then we move by a window's worth of lines.
            If, for example when moving up, the cursor is already near the top of the
            window then we move by a reduced number of lines in order than we not
            skip over any.
        """
        LinesInPage = int(self.TextWidget.winfo_height() / self.TextHeight)
        LinesFromBottom = LinesInPage - int(round((self.TextWidget.bbox(INSERT)[1] / float(self.TextHeight)))) - 1
        Delta = (LinesInPage / 2) + LinesFromBottom
    ...
    def on_KeyPrior(self,Event=None):
        ...
        """
        LinesInPage = int(self.TextWidget.winfo_height() / self.TextHeight)
        LinesFromTop = self.TextWidget.bbox(INSERT)[1] / self.TextHeight
        Delta = (LinesInPage / 2) + LinesFromTop
        self.TextWidget.mark_set(INSERT,'insert-%sl'%Delta)
        self.TextWidget.see(INSERT)
        self.TextWidget.ColorizeVisible()
        self.StatusUpdate()
    ...
    def on_Debug(self,Option):
        ...
        elif Option == 'DumpUndo':
            self.TextWidget.UndoDump()
        else:
    ...
    def on_KeyRelease(self,Event):
        ...
        """
        #D('on_KeyRelease, Event.keysym=%s'%Event.keysym)
        if not self.HaveKey:
            return
        self.HaveKey = False    
        ...
        if Event.keysym == 'Return' and not self.TextWidget.CursorInSystext():
            ...
            if Count:
                ...
                #if no spaces needed then generate an undo-sep. We need this because a sep is generated when we
                #   insert spaces but if no spaces are to be inserted we still need to generate the sep.
                self.TextWidget.UndoSeparator()
...
class BindEdit(Frame, BindMixin):
    ...
    def on_Assist(self,Event):
        ...
        if Temp == None:
            ...
            raise Exception, "Unexpected value = %s"%strx(Temp)    
...
class ResizeBar(Canvas):
    ...
    "f(NewLeftWidth, NewRightWidth)"
     ...
     o "VerifyCallback" is a function which we call when the user first clicks on a one of
       the resize knobs; it accepts a help topic argument. If it returns True we continue with the resize, else we don't allow
       the user to begin dragging the knob. This gives the Widgetator the opportunity to verify
       the current option before entering a Knob drag.
    ...
    def __init__(self, Master, Widths,  NotifyCallback, VerifyCallback):
        """=u
    ...
    def ResizeDown(self,Event):
        ...
        """
        if self.VerifyCallback('ResizeVarifyFailed') <> True:
            #If this was false we don't allow the drag to begin.
            return
...
class OptionEdit(Frame, BindMixin):
    ...
    *Note:* We make the assumption that at create time and at Reregister time that
    ...
    def RenameVisHandlers(self,A,B):
        ...
        if self.CurrentWidreq.PresentHome == GblParkingLotName or \
            ...
            for HandlerName in HandlerList:
                Temp = HandlerNameDecompose(HandlerName,A,Revise=True,NewWidgetName=B)
                if Temp:
                    #Name is standard form and refers to our previous name
                    self.CurrentWidreq.HandlerRenameNotify(HandlerName,Temp)
            #Have the widgetator update the editor if needed
            GblLayout.TheWidgetator.EditorRefresh()
            return

        TextEditor = GblLayout.TextEditFetch().TextWidget
        #--- Get a list of all handers in the form.
        HL = TextEditor.HandlerNameList()
        #Each entry in HandlerList is a list of:
            # [0] The new handler name
            # [1] The old handler name
        HandlerList = []
        for OldHandlerName in HL:
            NewHandlerName = HandlerNameDecompose(OldHandlerName,A,Revise=True,NewWidgetName=B)
            if NewHandlerName and NewHandlerName <> OldHandlerName:
                #Then the widget name and the handler name matched and we have a revised name.
                #We have to check to make sure that the old and new handler names are different because, although
                #  perverse, it is possible for them to be the same. If the user has a widget named "__A" which has
                #  a handler named "on_A_event", and they are renaming the widget to be "A" then our "new" handler
                #  name will be "on_A_event" which is the same as the new handler name. So if they are already
                #  the same we don't add them to the list.
                HandlerList.append([NewHandlerName,OldHandlerName])
        ...
        if ConflictCount == 0:        
            ...
            M = 'You just renamed the current widget from "$s" to "$s". The current form ' \
                ...
                'you want me to revise {the/those} handler name{/s} to match the revised name of the widget?'
            M = rpHelp.Plural(M,Count,ToPercent='$')
            M = M%(A,B,Rpo.WidgetNameSplit(A)[1])
            ...
            if Count <> ConflictCount:
                ...
                M = ('You just renamed the current widget from "$s" to "$s". The current form ' 
                    ...
                    'match the revised name of the widget BUT')
                M = rpHelp.Plural(M,Count,ToPercent='$')
                M = M%(A,B,Rpo.WidgetNameSplit(A)[1])
                ...
                M = ('You just renamed the current widget from "$s" to "$s". The current form ' 
                    'has %s event handler{/s} with {a name/names} like "on_$s_whatever". ' 
                    '\n\nAt this point I would usually ask if you want to rename {that/those} handler{/s} to '
                    'match the revised name of the widget BUT I also notice that if I did the rename then '
    ...
    def StatusCheck(self,HelpTopic=None):
        ...
        if HelpTopic and type(HelpTopic) not in (type(''),type(u'')):
        ...
        if ThisOption['EntryCode'] ==  'e':
            ...
            if NewValue == '' and OptionType<>'Name':
                ...
                #    check below and do nothing further. But if we *don't* display the value now then
        ...
        try:    
            ...
            #
                ...
                if R[0] <> 1:
                    print 'bbox R=%s'%strx(R)
                    raise Oops,R[0]
                else:
                    if NewValue <> None:
                        NewValue = BlankToNone(strx(R[1]))
    ...
    def Assist_Enumerated(self,Index):
        ...
        #So Escape will make the enumerated assist go away
        R.bind('<KeyPress-Escape>',self.EnumerateRadioToast)
        R.focus_set()
        ChoiceList.sort()    
    ...
    def on_Assist(self,Event):
        ...
        if OptionType == 'cvar':
            ...
            Result = Rpo.Assist_cvar(PresentValue,OptionName,ExtraInfo,WidreqName).Result
            if Result == None:
            ...
            if NewValue <> None:
                ...
                DisplayValue = Rpo.Format_cvar(NewValue,ExtraInfo,DefaultValue)
            ...
            R = Rpo.Assist_command(PresentValue,ExtraInfo,OptionName,ListOfHandlers,WidreqName).Result
            if R == None:
                #The user didn't do anything.
                return
            ...
            WidreqName = self.CurrentWidreq.Name
            Result = Rpo.Assist_pmwdatatype(PresentValue,OptionName,ExtraInfo).Result
            if Result == None:
                #The user didn't do anything.
                return
        ...
        #        
            ...
            NewValue = Cfg.Info['BuiltinOptions'][OptionType]['Assist'] \
                (ValueToEdit=PresentValue,OptionName=OptionName,Default=DefaultValue).Result
            if NewValue == None:
                #The user didn't do anything.
                return
            """
            #!!! Following is the old code disabled 2014-03-22.
            #Replaced by common code that follows
            if OptionType == 'font':
                DisplayValue = Rpw.FontToString(NewValue)
            elif OptionType == 'color':
                #Trim off the c:/v: prefix
                DisplayValue = NewValue[2:]
            else:    
                DisplayValue = NewValue = BlankToNone(NewValue)
            """
            DisplayValue = Cfg.Info['BuiltinOptions'][OptionType]['Format'](NewValue,ExtraInfo,DefaultValue)
            NewValue = BlankToNone(NewValue)
        else:
            raise Exception,'Unknown option type: '+OptionType
    ...
    def EnumerateRadioToast(self,Event=None):
        """=s
    ...
    def Generate(self,TargetSelectionName=None):
        ...
        for Opt,Info in Stuff:
            ...
            if Letters[0] == 'e':    
                ...
                """
                #!!Here is the old option-specific code which we disabled 2014-03-22.
                #It is replaced by the generic code that follows.
                #Delete this legacy code once the new code works
                if OptionType == 'cvar':
                    ...
                    #Trim off the c:/v: prefix
                    PresentValue = PresentValue[2:]
                """
                if IsBuiltin:
                    #Invoke formatting function for builtins
                    PresentValue = Cfg.Info['BuiltinOptions'][OptionType]['Format'](PresentValue,ExtraInfo,DefaultValue)
...
class Widgetator(Frame):
    ...
    def __init__(self,Master,**kw):
        ...
        self.ResizeBar = ResizeBar(NoteBookPage,self.OptionWidths,self.on_ResizeKnobs,self.StatusCheck)
...
class WidReq(DND.Dragged,BindMixin):
    ...
    *Variables*
    ...
    "self.FrameParent" : This variable is poorly named. It is the GuiEditor instance to which this widget belongs. 
                         It exists mostly so a widreq can call the FrameMark method of the GuiEditor as for 
                         example when a widreq is being selected and thus other frame widreqs need to unselect. ;
    "self.Icon"        : The icon we use to represent ourself. An object of type PhotoImage. ;
    ...
    def __init__(self
        ...
        self.Bindings = [] 
        self.Label = None
        ##D('Creating %s. I have no FrameParent yet.'%self.WRID_NameFetch())
        ...
        for Name in NameList:
            ...
            if Intify(Default) <> Intify(Current) or Type=='datatype':
                ...
                if Type in ('command','cvar'):
                    ...
                    #Color specification needs to be quoted, color variable not
                    if Current[:2] == 'c:':
                        Result.append((FudgedName,"'%s'"%Current[2:]))
                    else:
                        Result.append((FudgedName,Current[2:]))
                elif Type in ('dim','int'):
                    #Dim/int  number needs quoting, dim/int user-code does not
                    if Current[:2] == 'n:':
                        Result.append((FudgedName,"'%s'"%Current[2:]))
                    else:
                        Result.append((FudgedName,Current[2:]))
                elif Type == 'font':
                    #Fonts need a bit of special processing
                    if Current == None or Current == '':
                        #Request to use the default font; no action necessary
                        #Rpw.ErrorDialog('No action necessary')
                        pass
                    elif type(Current) in (type(''),type(u'')):
                        #This is user-text, user verbatim
                        Result.append((FudgedName,Current))
                        #Rpw.ErrorDialog(('Supplying verbatim user text'))
                    else:
                        #This is a font-tuple; convert to a string
                        Result.append((FudgedName,strx(Current)))
                        #Rpw.ErrorDialog(('Supplying converted tuple'))
                else:
                    """
                    ...
                    try:
                        ...
                        Result.append((FudgedName,"%s"%strx(Current)))
    ...
    def PackCode(self,Indent,Wrap,Side):
        ...
        "Side" The pack side. One of "top", "bottom", "left", "right". This is specified
            as an argument because it is computed by Rapyd, not set by the user.
    ...
    def CvarCode(self,Indent,Wrap):
        """
        Return the text needed to generate any cvars that we need.

        "Indent" is the indent factor.

        "Wrap" We wrap lines which would be longer than this.

        The result is a list of strings, or an empty list.
        """
        Result = []
        for Key in self.Options.keys():
    ...
    def IconBbox(self):
        """
        Return (X1, Y1, X2, Y2) bounding box of our icon on canvas.
        """
        return (self.X, self.Y, self.X+self.Icon.width(), self.Y+self.Icon.height())
...
class Layout(MenuAids):
    ...
    *Deletion* There are three ways in which widreqs can expire:
	...
	self._Win.geometry('+20+100')
        ...
        self.ActionMenuSep()
        self.ActionMenuAdd('Preferences','ProjectConfig')
    ...
    def on_ActionProjectConfig(self,Evant=None):
        """
        Used clicked the Config choice
        """
        Help('Project.Config')
    ...
    def on_ActionProjectInfo(self,Event=None):
        ...
        for Mod,Lines in GblProjectInfo:
            Msg.append((Mod,strx(Lines)))
            Total += Lines
        Msg.append(('','-----'))
        Msg.append(('Total:',strx(Total)))    
    ...
    def on_ActionProjectNew(self,SaveOption=True,PossibleName=''):
        ...
        if SaveOption:
            if self.SaveOption('New'):
                ...
                return False
        #If PossibleName contains a directory we use it
        Head, Tail = os.path.split(PossibleName)
        if Head <> '':
            #It does contain a directory
            Dir = Head
            PossibleName = Tail
        else:
            #It doesn't contain a directory; pick a directory per the config file
        ...
        while 1:
            ...
            if not os.path.isdir(R[0]):
                #The directory doesn't exist; ask the user if we should create it
                Msg = 'Hi there. You just asked me to create project "%s" in directory "%s" but that directory ' \
                    'does not exist. Would you like me to create that directory now?'%(R[1],R[0])
                R2 = Rpw.MessageDialog(Title='Query', Message=Msg, Buttons=(('Create directory',1),('~Cancel',None)),
                    Help=('project.new.nodir',(R[0],R[1])), Justify=LEFT).Result
                if R2 <> 1:
                ...
                try:
                    ...
                    #Directory creation failed; tell user
                    Msg = 'Alas, I encountered an error while attempting to create the directory. Click HELP for more details.'
                    Rpw.MessageDialog(Title='Bad News',Message=Msg,Help=('project.new.mkdir.failed',(R[0],R[1],strx(E),)))
                    continue
            else:
                #The directory exists, carry on
                pass
            #We have a directory for the project. Now lets make sure we have write-access. To do this we write an empty version
            #    of the ".dec" file just to see if it works.
            try:
                F = open('%s%s.dec'%tuple(R),'w')
                #It worked
                F.close()
                #That worked, were done
                break
            except Exception as E:
                #It didn't work.
                Msg = 'I encountered an error while attempting to create the new project in the specified directory. ' \
                    'Click HELP for more details.'
                Rpw.MessageDialog(Title='Bad News',Message=Msg,Help=('project.new.write.failed',(R[0],R[1],strx(E),)))
                continue
        #The directory exists and we have write access, keep going
        Cfg.Info['ProjectDir'], ProjectName = R
        APD()
        self.ProjectNewBuild(ProjectName,Cfg.Info['ProjectDir'])    
    ...
    def ProjectNewBuild(self,ProjectName,ProjectDir):
        ...
        if ProjectDir <> '' and ProjectDir[-1] <> os.path.sep:
        ...
        self.WRID = Repo.Register('Layout area')
        self.ModuleNameUpdate()
        self.ApplicationTitleSet(ProjectDir+ProjectName)
    ...
    def on_ActionModuleRename(self):
        ...
        NewName = Rpw.ModuleNameDialog(Modules=GblModules.keys(),Current=OldName
            ,Forms=Forms,ProjectDirectory=Cfg.Info['ProjectDir']).Result
        if NewName == None:
            return
        ...
        if OldName == Cfg.Info['ProjectName']:
            ...
            self.ApplicationTitleSet(Cfg.Info['ProjectDir']+NewName)
    ...
    def on_ActionModuleNew(self):
        ...
        if not self.TheWidgetator.StatusCheck():
            #There are invalid edit values
            return
        Temp = Rpw.ModuleNameDialog(Modules=GblModules.keys(),ProjectDirectory=Cfg.Info['ProjectDir']).Result        
        if Temp == None:
            return
    ...
    def on_ActionFormExport(self,Event=None):
        ...
        #Kluge for testing
        F = open('temp.txt','w')
        T = '\n'.join(self.FormDecode(FormInfo))
        F.write(T)
        F.close()
        #End kluge
        ImportTypes = self.ProjectFetchImportTypes()
        Data = ('Form',RpjFileVersionNumber,FormInfo,ImportTypes)

        File = tkFileDialog.asksaveasfile(parent=self._Win
            ,initialdir=Cfg.Info['ProjectDir']
            ,initialfile=Form
            ,title='Export Form'
            ,filetypes=[('Rapyd forms','*.rpf'),('Rapyd projects','*.rpj'),('Rapyd forms+projects','*.rpf *.rpj'),('All Files','*')]
            )
        if not File:
            #User bailed
            return 
    ...
    def on_ActionFormImport(self,Event=None):
        ...
        """        
        File = tkFileDialog.askopenfile(parent=self._Win
            ,title='Import Form'
        ...
        try:
            ...
            assert len(Info) == 4
            assert Info[0] == 'Form'
            if Info[1] > RpjFileVersionNumber:
                Msg = 'File version conflict. Click Help for further information.'
                Rpw.MessageDialog(Title='Problem',Message=Msg,Help=('Import.Failed',['0', '1', strx(Info[1]), VersionNumber]))
                return
            #We will need the FileVersion at FormLoad time
            FileVersion = Info[1]
            FormInfo = Info[2]
            ...
            #And now for a whole pile of checking.
            ...
            Temp = self.VetOptionValues(FormInfo['Widreqs'],ModuleName,FormName,FileVersion)
            ...
            self.FormLoad(FormInfo,FileVersion)
            ...
            Rpw.MessageDialog(Message='An error was encountered while attempting to import the Form.'+str(Message)
                ,Widget=self._Win)
    ...
    def ProjectBuild(self,Run=False):
        ...
        DecFilename = '%s%s.dec'%(Cfg.Info['ProjectDir'],Cfg.Info['ProjectName'])
        ...
        if Run:
            ...
            ResultFN = '%s%s.result'%(Cfg.Info['ProjectDir'],Cfg.Info['ProjectName'])
            try:
                os.remove(ResultFN)
            except:
                pass    
            ...
            FN = '%s%s.py'%(Cfg.Info['ProjectDir'],Cfg.Info['ProjectName'])
            ##D('About to run project...')
            ##D(Cfg.Info['ProjectDir'])
            ##D(Cfg.Info['ProjectName'])
            
            if Cfg.Info['ProjectDir'] <> '':
                os.chdir(Cfg.Info['ProjectDir'])
            try:
                ...
                Dlg.Close()
                R = rpErrorHandler.RunError(Info=LocatorDict,ProjectDirectory=Cfg.Info['ProjectDir']).Result
                if R == None:
                    #No action required
                    pass
                elif R == 1:
    ...
    def VetOptionValues(self,WidreqList,ModuleName,FormName,FileVersion):
        """
        ...
        This is also where we look after file version conversion. In particular:

            o Colors. Starting with V1 we now prefix colors with c: or u:.
            o Fonts. Can now be tuples or user-specified strings.
        """
        ...
        for Widreq in WidreqList:
            ...
            DonorModule = Widreq['ModuleName']
            #Do a pass over pack-options updating any old-format dims/ints to new format
            for J in range(len(Widreq['PackOptions'])):
                Option = Widreq['PackOptions'][J]
                Type,Default,Current,Extra = Cfg.Info['PackOptions'][Option['Name']]
                if Type in ('dim','int'):
                    #Add prefix to any old style dim or int value
                    if Option['Value'][:2] not in ('u:','n:'):
                        Widreq['PackOptions'][J]['Value'] = 'n:' + Option['Value']
            #Now do a pass over the regular options where much more checking is required
            #  because of possible chanages in the config file.
            ...
            for J in Temp:
                ...
                if FileVersion == 0:
                    if Type == 'font':
                        #In file version-0 we kept font information as stings with less than stellar results.
                        #Now we keep font information thusly:
                        #  Empty string => Use the default font
                        #  Non-empty string => Use this user-supplied text verbatim
                        #  3-tuple => here is a font specification tuple
                        #If we are reading a version-0 file then we attempt to convert the font specification
                        #  into a font tuple.
                        FT = Rpw.StringToFont(Option['Value'])
                        if type(FT) == type(()):
                            #It converted OK, use the tuple
                            Widreq['Options'][J]['Value'] = FT
                    elif Type == 'color':
                        #In file version-0 all color information was what we now call c:color.
                        Widreq['Options'][J]['Value'] = 'c:' + Option['Value']
                if Type in ('dim','int'):
                    #Add prefix to any old style dim or int value
                    if Option['Value'][:2] not in ('u:','n:'):
                        Widreq['Options'][J]['Value'] = 'n:' + Option['Value']
                try:
                    Value = Option['Value']
                    ...
                    Result.append((ModuleName,FormName,WidreqName,Option['Name'],strx(Value),strx(Default)))
        return Result            
    ...
    def ProjectLoad(self,ProjectPath=None):
        ...
        """
        global GblPreviousModule, GblLoadInProgress, GblLastSavedProject, GblProjectInfo
        if ProjectPath == None:
            Dir = Cfg.Info['DefaultDir']
            if Dir == '*' or not os.path.isdir(Dir):
                #Either the config said to use the current working directory or the directory
        ...
        try:
            ExceptionReason = ''
            F = open(ProjectPath)
            ExceptionReason = 'File is corrupt or not a Rapyd-Tk project'
            ...
            if ProjectInfo['Version'] > RpjFileVersionNumber:
                Msg = 'File version conflict. Click Help for further information.'
                #Help args are:
                #  Lowest rpj file version we handle
                #  Highest rpj file version we handle
                #  Version number of file being loaded
                #  Rapyd version number
                Rpw.MessageDialog(Title='Problem',Message=Msg,Help=('Load.VersionConflict',['0', '1', strx(ProjectInfo['Version'])
                    ,VersionNumber]))
                return 2
            #Make sure the project wasn't renamed by the operating system
            Path, Name = os.path.split(ProjectPath)
            Name, Ext = os.path.splitext(Name)
            if Name <> ProjectInfo['ProjectName']:
                Msg = 'Project "%s.rpj" is in the midst of an identity crisis and thus can not be loaded. For further information ' \
                    'and to find out what you can do, press HELP.'%Name
                Rpw.MessageDialog(Title='Problem',Message=Msg,Help=('Load.identity.crisis',[Path,Name,ProjectInfo['ProjectName']]))
                return 2
            ...
            if TheDir <> '' and TheDir[-1] <> os.path.sep:
                TheDir += os.path.sep
            ...
            for ModuleDescriptor in Project[1:]:
                ...
                for FormDescriptor in ModuleDescriptor['Forms']:
                    ...
                    if Temp:
                        AlteredFormDict[(ModuleName,FormName)] = None
                    Temp = self.VetOptionValues(FormDescriptor['Widreqs'],ModuleName,FormName,ProjectInfo['Version'])
                    ResetOptionList.extend(Temp)
                    ResetOptionCount += len(Temp)
                    if Temp:
                        AlteredFormDict[(ModuleName,FormName)] = None
            ...
            for OneModule in Project[1:]:
                ...
                for Info in OneModule['Forms']:
                    ...
                    self.FormLoad(Info,ProjectInfo['Version'])
            ...
            try:
                ...
                pass
            Msg = 'An error was encountered while attempting to load the project.'
            Rpw.MessageDialog(Message=Msg, Widget=self._Win, Help='Load.Failed')
            return 2

    def FormLoad(self,Info,FileVersion):
        """
        ...
        for WidreqInfo in Info['Widreqs']:
            ...
            """
            #!! I believe this code is now (2014-03-23) redundant. Take it out after testing.
            #In file version-0 we kept font information as stings with less than stellar results.
            #Now we keep font information thusly:
            #  Empty string => Use the default font
            #  Non-empty string => Use this user-supplied text verbatim
            #  3-tuple => here is a font specification tuple
            #If we are reading a version-0 file then we attempt to converr the font specification
            #  into a font tuple.
            if FileVersion == 0:
                for OurOption  in WidreqInfo['Options']:
                    if OurOption['Name'] == 'font':
                        #Attempt to normalize the font string
                        FT = Rpw.StringToFont(OurOption['Value'])
                        if type(FT) == type(()):
                            #It converted OK, use the tuple
                            OurOption['Value'] = FT
            """
    ...
    def FormDecode(self,Form,Indent=0):
        """
        Given a dictionary of form information return a list of strings representing it.
    ...
    def ProjectDecode(self,Project):
        ...
        R.append('    EditorWidths: %s'%strx(Preamble['EditorWidths']))
    ...
    def ProjectSave(self):
        ...
        #Pickle it to disk
        FN = '%s%s'%(Cfg.Info['ProjectDir'],Cfg.Info['ProjectName'])
        F = open(FN+'.$$$','w')
    ...
    def ModuleBuild(self,ModuleName):
        ...
        Filename = '%s%s.py'%(Cfg.Info['ProjectDir'],ModuleName)
        try:
            os.remove(Filename)
        except:
            pass    
        ...
        for Herd in HerdList:
            if Herd in ('the Parking Lot',GblMainName):
                ...
                #This "pass" statement solves a little problem. If the user makes a syntax error in the last line of
                #  a form then python reports in on the *subsequent* line. Adding this "pass" statement ensures
                #  that we report the error within the form which contains the erronious line.
                Text += '\npass #---end-of-form---\n'
    ...
    def on_LayoutAreaRaise(self,FormName):
        ...
        if R <> 1:
            print 'Problem selecting form: %s'%strx(R)
    ...
    def FormSystextHeaderGenerate(self,FormName,BaseClass,ModType):
        ...
        TemplateE = '%sdef __init__(self,Master=None,*pos,**kw):\n'%Indent
    ...
    def FormCreateNew(self,Info,OfMainModule=False):
        ...
        if NewName == GblMainName:
            ...
            TextEditor.insert(END,GenerateImportLines(['Tkinter']),'bgldt',PowerPill=1)
            ...
            if IsMainModule:
                ...
                TextEditor.insert(END,InitCode,'bgldt',PowerPill=1)
            ...
            if not GblLoadInProgress:
                ...
                TextEditor.insert('1.0',ST[0],'bgldt',PowerPill=1)
                TextEditor.insert(END,TemplateA,'')
                TextEditor.insert(END,ST[1],'bgldt',PowerPill=1)
                TextEditor.insert(END,TemplateA,'')
                TextEditor.insert(END,TemplateB,'bgldt',PowerPill=1)
                TextEditor.insert(END,'\n','')
                TextEditor.insert(END,TemplateC,'bgldt',PowerPill=1)
    ...
    def on_ConfigureAfter(self):
        ...
        if FormInfo.has_key('Gui') and FormInfo['Gui'] <> None:
            ...
            self._Win.update_idletasks()
            Size = self.NoteBookSize()
            #D('on_Configure NoteBookSize=%s'%strx(Size))
            if Size <> (1,1):
...
class GuiEditor(rpDndCanvasWid,BindMixin):
    ...
    *Note:* A great many routines depend on the fact that bounding boxes are represented
    ...
    def Create(self,Indent,Wrap,Whine):
        ...
        #    While we are scanning through all our widreqs we have them generate
        #    the code necessary to create any automatic cvars so they will be
        #    created before any widgets that need to reference them.
        XDict = {}
        #We use this dict to prevent duplicate generation of cvars
        CvarDict = {}
        for WidReqName in Repo.ListWidreqs(self.HerdName):
            ...
            if WidReq.FrameID <> None:
                ...
                #This is a containee widreq
                T = WidReq.CvarCode(Indent,Wrap)
                for TLine in T:
                    if not CvarDict.has_key(TLine):
                        #Not a duplicate
                        Result.append(TLine)
                        CvarDict[TLine] = None
        ...
        for FrameID in FrameList:
            ...
            if len(Info[FrameID]['WidReqs']) == 1:
                ...
                #    relatively closest to. The relative part is important in the case
                #    of a long narrow frame. For each direction we compute the percentage
                #    of where the widreq is within the space available in that direction
                FrameW = float(Bbox[2] - Bbox[0])
                FrameH = float(Bbox[3] - Bbox[1])
                W = Repo.Fetch(Info[FrameID]['WidReqs'][0],self.HerdName)
                #W is our widget. Have it tell us about it's extents
                WL, WT, WR, WB = W.IconBbox()
                DistanceList = [[(WL - Bbox[0])/FrameW,'left']
                               ,[(Bbox[2] - WR)/FrameW,'right']
                               ,[(WT - Bbox[1])/FrameH,'top']
                               ,[(Bbox[3] - WB)/FrameH,'bottom']]
                DistanceList.sort()
                ...
                while len(Wlist) >= 1:
                    ...
                    #    closest, pop it from the list and carry on. Note that if there is 
    ...
    def FrameSanityCheck(self,Description):
                ...
                if FrameDict.has_key(WidReq.FrameID):
                    ...
                    for ID,NameOfWidreq in FramesProcessed:
                        if ID == WidReq.FrameID:
                            ...
                            print('   Error: Widreqs %s and %s both refer to frame %s'%(NameOfWidreq,WidReqName,strx(ID)))
                            ErrorCount += 1
                            break
                    else:
                        ...
                        if ErrorCount == 0:
                            print('-----FrameSanityCheck %s-----'%Description)
                        print('   Error: WidReq "%s" referenced Frame %s which does not exist.'%(WidReqName,strx(WidReq.FrameID)))
                        ErrorCount += 1
        ...
        if len(FrameDict) <> 0:
            ...
            for ID in FrameDict.keys():
                print('   Error: There is no frame WidReq which references Frame %s'%strx(ID))
                ErrorCount += 1
        if ErrorCount == 0:
            pass
        ...
        for WidReqName in Repo.ListWidreqs():
            WidReq = Repo.Fetch(WidReqName)
            if WidReq.FrameID <> None:
                print('      Widreq %s, FrameID %s'%(WidReqName,strx(WidReq.FrameID)))
        ...
        for WidReqName in Repo.ListWidreqs():
            ...
            if WidReq.FrameID <> None:
                ...
                try:
                    ...
                    print 'Widreq.FrameID=%s'%strx(WidReq.FrameID)
                    print 'Info[0]=%s'%strx(Info[0])
    ...
    def FrameInfoFormat(self,FrameInfo=None):
        ...
        for ID,Info in FrameInfo[0].items():
            Result.append('%s: %s'%(strx(ID),strx(Info)))
        for Item in FrameInfo[1]:
            Result.append(strx(Item))
        return '\n'.join(Result)        
    ...
    def EnclosingFrameFind(self,CanvasXY,FrameDict):
        ...
        raise Exception, "Point %s not found in any frame"%strx(FrameDict)            
    ...
    def FramesRemediate(self,BboxDict,Mode):
        ...
        if Mode == 'd':
            ...
            #   be 1 or 2.
            if len(BboxDict) <> 0:
                print 'BboxDict=%s'%strx(BboxDict)
    ...
    def on_Button1Up(self,Event):
        ...
        if self.State == GuiEditor.StateLinePlace or self.State == GuiEditor.StateLineMove:
            ...
            assert W <> None, 'Continer WidReq not found; ID=%s'%strx(TargetFrameID)
            #---Select that widreq---
...
class ConfigReader:
    """=m
    Reads the config file and stores it in dictionary self.Info. This dictionary
        also holds bits of information that we want to have around globally even
        though technically it doesn't come from the config file.
    ...
    o ["'HelpWindowSize'"] An integer in the range 0-100 saying what the target
        initial help window size is as a percentage of the full screen size
    ...
    o ["'ManglePrefix'"] This is set by the "AutoMangle" option and will be "__"
        if AutoMangle was 1, "" if AutoMangle was 0 and '_' if AutoMangle was 2.
        It is prepended to names of widgets, eventhandlers etc.
    ...
    def __init__(self):
        ...
        ConfigFilename = os.path.realpath(ConfigFilename)
        ...
        #Create links to the Valaidate, Assist and Format functions for each type
        for Temp in ConfigReader.BuiltinOptionTypes.keys():
            self.Info['BuiltinOptions'][Temp]['Validate'] = eval('Rpo.Validate_%s'%Temp)
            self.Info['BuiltinOptions'][Temp]['Assist']   = eval('Rpo.Assist_%s'%Temp)
            try:
                #If this type has a specific format function then use it
                self.Info['BuiltinOptions'][Temp]['Format']   = eval('Rpo.Format_%s'%Temp)
            except AttributeError:
                #But if there is no specific function the use the default format function
                self.Info['BuiltinOptions'][Temp]['Format']   = eval('Rpo.Format_default')
        self.ErrorCount = 0
        ...
        if not self.C.has_section(Name):
            ...
            if LinkColor:
                #Color validator expects color/variable prefix so we supply one to keep it happy
                R = Rpo.Validate_color('c:'+LinkColor,'')
                if R[0] <> 1:
            ...
            #Help window size
            self.Info['HelpWindowSize'] = 60
            Temp = self.Fetch('General','HelpWindowSize')
            try:
                Temp = int(Temp)
                if Temp < 0 or Temp > 100:
                    self.Error('Section "General", "HelpWindowSize" value "%s" is not in the range 0..100'%Temp)
                else:
                    self.Info['HelpWindowSize'] = Temp
            except ValueError:
                self.Error('Section "General", "HelpWindowSize" specification "%s" is not an integer'%Temp)
            #make sure directory name ends in a slash
            if self.Info['IconDir'][-1] <> os.path.sep:
                self.Info['IconDir'] += os.path.sep
            # The AutoMangle option
            Temp = self.Fetch('General','AutoMangle')
            try:
                AM = int(Temp)
            except:
                #Set an invalid value so we whine
                AM = 'x'
            if AM == 0:
                ...
                self.Info['ManglePrefix'] = '_'
            else:
                self.Error('Section "General" invalid AutoMangle "%s"'%Temp)
            ...
            if len(T) == 2 and T[0] == 'EditorScheme':
                ...
                for Option in Options:
                    if Option[:6] == 'Color_':
                        ...
                        #
                        Color = Option[6:]
                        #Use the option editor color validater. Usually our color specification are prefixed with "c:" or "v:"
                        #    but in this case it is always a color so we supply the "c:"
                        Temp = Rpo.Validate_color('c:'+Color,None)
                        if Temp[0] <> 1:
                            #Color didn't fly
                            Msg = 'Section "%s", option "%s", %s'%(Section,Option,Temp[0])
                            self.Error(Msg)
                            continue
                        #We have a valid color. Strip off the "c:".
                        Color = Temp[1][2:]
                        ...
                        for Ac,Es in Scheme['Actions']:
                            if Ac == Action:
                                Msg = 'Section "%s", option "%s": duplicate specification of action "%s"' \
                                    %(Section,Option,Action)
                                self.Error(Msg)
                                continue    
                            if Es == EventString:
                                Msg = 'Section "%s", option "%s": duplicate use of event-string "%s"' \
                                    %(Section,Option,EventString)
                                self.Error(Msg)
                                continue
                        #We like the specification; add to scheme
                        Scheme['Actions'].append((Action,EventString),)                        
                    elif Option == 'Font':
                        Font = self.Fetch(Section,Option)                
                        Temp = Rpo.Validate_font(Font,None)
                        if Temp[0] <> 1:
                            #Font didn't fly
                            Msg = 'Section "%s", option "%s", %s'%(Section,Option,Temp[0])
                            self.Error(Msg)
            ...
            for Option in Options:
                ...
                if TT[0] == 'font':    
                    try:
                        ...
                        if FontInfo <> None:
                            if len(FontInfo) < 2:
                                raise Exception('FontError')
                            for F in FontInfo:
                                if type(F) not in (type(''),type(u'')):
                                    raise Exception('FontError')
                            int(FontInfo[1])
                ...
                if self.Info['BuiltinOptions'].has_key(TT[0]):
                    ...
                    if TT[0] == 'color':
                        #For color specification we need to prepend 'c:' or the validator will complain.
                        TT[1] = 'c:' + TT[1]
                    elif TT[0] in ('dim','int'):
                        #For dimensioned and integer numbers we need to prepend 'n:'
                        TT[1] = 'n:' + strx(TT[1])
                    ValidationArg = TT[1]
...
"""
Cfg.Info['ProjectDir']. Is set at "Load Project" and "New Project" time. Is guaranteed to end
...
Help = rpHelp.TheHelpThingy(
    ...
    ,WindowSize = Cfg.Info['HelpWindowSize']
    )
Rpw.Help = Help
...
Rpw.CopyrightDate = CopyrightDate
