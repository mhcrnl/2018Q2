...
How to a add a new built-in option to Rapyd:
    ...
    o If the new type needs formatting before being presented to the user then write
      a Format routine. This does NOT have to exist. If you don't write a specific
      Format then the routine Format_default is used which leaves the option value
      untouched.
    ...
    The above assumes that the new type requires no special processing. If
        ...
        processes changed entry values (routine StatusCheck), where it deals
        with assists (routine on_Assist) and where a WidReq helps with the
        code to create an instance (routine WidReq.CreationList).
...
def ArgFetch(kw,Name,Default=None,Required=False):
    """
    Fetch a keyword argument from a kw dictionary

    Name is a string giving the name of the argument.

    Normally the argument is optional and we supply Default if it is omitted.
    If Required is True then we raise an error if the argument is omitted.

    Result is the argument value passed (or Default).
    """
    if kw.has_key(Name):
        #The keyword argument was supplied
        Result = kw[Name]
        del kw[Name]
    else:
        #The keyword argument was not supplied
        if Required:
            raise Exception,'Required keyword argument "%s" not supplied.'%str(Name)
        Result = Default
    return Result

def Format_default(Arg,Extra,Default):
    """
    This is a null format routine for types that don't need formatting
    """
    return Arg

def WidgetNameSplit(Name):
    """
    Split a widget name into its leading-underscore and name parts.
    """
    if len(Name) > 2 and Name[:2] == '__':
        Result = ['__', Name[2:]]
    elif len(Name) > 1 and Name[:1] == '_':
        Result = ['_', Name[1:]]
    else:
        Result = ['', Name]
    return Result
...
def Validate_xy(Arg,OptionName):
    """
...
class Assist_xy:
    ...
    """
    def  __init__(self,ValueToEdit,OptionName,Default):
    ...
    def on_OK(self,Event=None):
        ...
        Temp = Validate_xy(NewValue,self.OptionName)
        if Temp[0] <> 1:
...
def Validate_verbatim(Arg,OptionName):
    """
...
#end Validate_verbatim

class Assist_verbatim:
    """
...
def Validate_text(Arg,OptionName):
    """
...
def Validate_tabs(Arg,OptionName):
    """
    ...
    for Temp in Arg.split():
        ...
        R = Validate_dim(Temp,OptionName)
        if R[0] == 1:
...
class Assist_tabs:
    ...
    def on_OK(self,Event=None):
        NewValue = self.Entry.get()
        Temp = Validate_tabs(NewValue,self.OptionName)
        if Temp[0] <> 1:
            Rpw.ErrorDialog(Temp[0])
            return
        self.Result = str(Temp[1])
...
def Validate_pyclass(Arg,OptionName):
    """
...
#end Validate_pyclass

class Assist_pyclass:
    """
    There is no assist for this type of option.

    This dummy assist exists only so we can create a list of validation and
        assist routines in the main program without having to special case
        this type.
    """
...
def Validate_pmwdatatype(Arg,OptionName):
    """
...
#end Validate_pmwdatatype

class Assist_pmwdatatype:
    """
    ...
    def  __init__(self, ValueToEdit, OptionName, ExtraInfo):
        ...
        #The format of cvar ExtraInfo is described with Format_cvart
        #We want to position ourself near the mouse
        MouseX, MouseY = self._Win.winfo_pointerxy()
...
def Format_pmwcomp(Arg, Extra, Default):
    """
    For pmwcomp the value to display comes from the default field.

    I would say more here, but the "why" of this has been lost in the mists of time.
    """
    return Default

def Validate_pmwcomp(Arg,OptionName):
    """
...
#end Validate_pmwcomp

class Assist_pmwcomp:
    """
    There is no assist for this type of option.
...
def Format_int(Arg, Extra, Default):
    """
    Format integer number for presentation to user
    """
    return Arg[2:]

def Validate_int(Arg,OptionName):
    """
    Verify that we like an integer number value
    ...
    """
    Arg = str(Arg).strip()
    if len(Arg) < 2 or not Arg[:2] in ('n:','u:'):
       return ('Integer number specification doesn\'t open with "n:" or "u:"',Arg)
    Prefix = Arg[:2]
    Detail = Arg[2:]
    if Prefix == 'n:':
        if len(Detail) == 0:
            return ('Blank is not a valid integer number',Detail)
        try:
            int(Detail)
            return (1,Arg)
        except:
            return ('"%s" is not a valid integer number.'%Detail,Detail)
    else:
        if len(Detail) == 0:
            return ('User code specification can not be blank',Detail)
    return (1,Arg)

    #end Validate_int

class Assist_int(Toplevel):
    def __init__(self,Master=None,**kw):
        """
        The integer number edit assist.

        ValueToEdit is the initial value to show the user. It is taken, unchecked, from
            the Entry widget.

        If the result is None then the user cancelled.

        Otherwise the result is a tuple:
            [0] 'n' if they specified a number, 'u' if they specified arbitrary user code
            [1] The text of the number or code

            If they specified a number then we have called the Validate_int routine to
            make sure it is valid.

            If they specified arbitrary user code then no checks have been done and it's their
            problem if it bombs.

        This dialog was developed using Rapyd-Tk. Bootstrapping rocks.
         """
        T = ArgFetch(kw,Name='ValueToEdit',Required=True)
        self.TypeFlag = T[:2]
        self.ValueToEdit = T[2:]
        self.OptionName = ArgFetch(kw,Name='OptionName',Required=True)
        self.Default = ArgFetch(kw,Name='Default',Required=True)
        self.SelectedButton = IntVar()
        assert self.Default[:2] in ('n:','u:'),self.Default

        apply(Toplevel.__init__,(self,Master),kw)
        self.bind('<KeyPress-Escape>',self.__on_Assist_int_Key_Escape)
        self.bind('<KeyPress-Return>',self.__on_Assist_int_Key_Return)
        self.__Frame5 = Frame(self)
        self.__Frame5.pack(anchor='w',side='top')
        self.__Label4 = Label(self.__Frame5,anchor='w',text='Integer Number')
        self.__Label4.pack(anchor='w',padx=10,pady=10,side='top')
        self.__Frame3 = Frame(self,borderwidth=3,relief='raised')
        self.__Frame3.pack(fill='x',padx=10,pady=10,side='top')
        self.__Radiobutton1 = Radiobutton(self.__Frame3
            ,command=self.__on_Radiobutton1_command,value=1,variable=self.SelectedButton)
        self.__Radiobutton1.pack(side='left')
        self.__Label1 = Label(self.__Frame3,text='Use this value:')
        self.__Label1.pack(pady=20,side='left')
        self.__EntryNumber = Entry(self.__Frame3)
        self.__EntryNumber.pack(side='left')
        self.__EntryNumber.bind('<ButtonRelease-1>',self.__on_EntryNumber_ButRel_1)
        self.__ButtonDefault = Button(self.__Frame3,text='Default',width=13)
        self.__ButtonDefault.pack(padx=10,pady=5,side='left')
        self.__ButtonDefault.bind('<ButtonRelease-1>',self.__on_ButtonDefault_ButRel_1)
        self.__Frame2 = Frame(self)
        self.__Frame2.pack(side='top')
        self.__Label3 = Label(self.__Frame2,text='--- OR ---')
        self.__Label3.pack(pady=10,side='top')
        self.__Frame4 = Frame(self,borderwidth=3,relief='raised')
        self.__Frame4.pack(fill='x',padx=10,pady=10,side='top')
        self.__Radiobutton2 = Radiobutton(self.__Frame4
            ,command=self.__on_Radiobutton2_command,value=2,variable=self.SelectedButton)
        self.__Radiobutton2.pack(side='left')
        self.__Label2 = Label(self.__Frame4,text='Use this code:')
        self.__Label2.pack(pady=20,side='left')
        self.__EntryUser = Entry(self.__Frame4,width=35)
        self.__EntryUser.pack(side='left')
        self.__EntryUser.bind('<ButtonRelease-1>',self.__on_EntryUser_ButRel_1)
        self.__Frame1 = Frame(self)
        self.__Frame1.pack(fill='x',side='top')
        self.__ButtonOK = Button(self.__Frame1,text='OK')
        self.__ButtonOK.pack(expand='yes',pady=20,side='left')
        self.__ButtonOK.bind('<ButtonRelease-1>',self.__on_ButtonOK_ButRel_1)
        self.__ButtonHelp = Button(self.__Frame1,text='Help')
        self.__ButtonHelp.pack(expand='yes',side='left')
        self.__ButtonHelp.bind('<ButtonRelease-1>',self.__on_ButtonHelp_ButRel_1)
        self.__ButtonCancel = Button(self.__Frame1,text='Cancel')
        self.__ButtonCancel.pack(expand='yes',side='right')
        self.__ButtonCancel.bind('<ButtonRelease-1>',self.__on_ButtonCancel_ButRel_1)
        #
        #Your code here
        #
        self.title(Rph.Cap(self.OptionName))

        #This is our default result in case we close unexpectedly
        self.Result = None

        #Look after initial setup.
        if self.TypeFlag == 'n:':
            #We have a number specification
            self.__Radiobutton1.select()
            self.__EntryNumber.insert(0,self.ValueToEdit)
            self.CurrentNumber = self.ValueToEdit
            self.__EntryNumber.focus_set()
        else:
            #We have arbitrary code
            self.__Radiobutton2.select()
            self.__EntryUser.insert(0,self.ValueToEdit)
            self.__CurrentNumber = self.ValueToEdit
            self.__EntryUser.focus_set()

        self.protocol('WM_DELETE_WINDOW',self.__on_ButtonOK_ButRel_1)
        #Be modal
        Rph.Grabber(self)
        self.wait_window()
    ...
    def __on_Assist_int_Key_Escape(self,Event=None):
        """
        User pressed Escape
        """
        self.__on_ButtonCancel_ButRel_1(Event)

    def __on_Assist_int_Key_Return(self,Event=None):
        """
        User pressed Return
        """
        self.__on_ButtonOK_ButRel_1(Event)

    def __on_ButtonCancel_ButRel_1(self,Event=None):
        """
        User clicked on Cancel
        """
        self.destroy()

    def __on_ButtonDefault_ButRel_1(self,Event=None):
        """
        User asked to revert to the default value
        """
        self.__Radiobutton1.select()
        self.__EntryNumber.delete(0,END)
        self.__EntryNumber.insert(0,self.Default[2:])
        self.CurrentNumber = self.Default[2:]
        self.__EntryNumber.focus_set()
    ...
    def __on_ButtonHelp_ButRel_1(self,Event=None):
        ...
        """
        Help('assist.int',[self.OptionName,self.Default[2:]])


    def __on_ButtonOK_ButRel_1(self,Event=None):
        """
        User clicked on OK
        """
        if self.SelectedButton.get() == 1:
            #They selected a number
            NewValue = 'n:' +self.__EntryNumber.get()
        else:
            #They selected arb code
            NewValue = 'u:' +self.__EntryUser.get().strip()
        T = Validate_int(NewValue,self.OptionName)
        if T[0] <> 1:
            #We don't like their specification
            Rpw.ErrorDialog(T[0])
            return
        #We like their specification
        self.Result = T[1]
        self.destroy()

    def __on_EntryNumber_ButRel_1(self,Event=None):
        """
        User clicked on the number entry
        """
        self.__Radiobutton1.select()

    def __on_EntryUser_ButRel_1(self,Event=None):
        """
        User clicked on the variable entry
        """
        self.__Radiobutton2.select()

    def __on_Radiobutton1_command(self,Event=None):
        """
        User clicked on number radio button
        """
        self.__EntryNumber.focus_set()

    def __on_Radiobutton2_command(self,Event=None):
        """
        User clicked on the user code radio button
        """
        self.__EntryUser.focus_set()
    #
...
def Validate_image(Arg,OptionName):
    """
...
class Assist_image:
    ...
    def on_OK(self,Event=None):
        NewValue = self.Entry.get()
        Temp = Validate_image(NewValue,self.OptionName)
        if Temp[0] <> 1:
            Rpw.ErrorDialog(Temp[0])
            return
        self.Result = str(Temp[1])
...
def Format_font(Arg, Extra, Default):
    """
    Format our font specification.
    """
    return Rpw.FontToString(Arg)

def Validate_font(Arg,OptionName):
    """
    Verify that we like a font specification.

    The specification should be either a tuli of (Family, Size, Modifiers), an
        X style font string or an empty string.
    ...
    Note that when supplying a font for a widget a font of None is acceptable
        (presumably indicating a default font) but "''" is not acceptable. However
        here we accept either.
    """
    if Arg == None or type(Arg) == type(''):
        #None or strings we accept
        Result = (1,Arg)
    elif type(Arg) == type(()):
        try:
            Button(font=Arg)
            ...
            Result = ('"%s" is not an acceptable font descriptor.'%str(Arg),Arg)
    else:
        raise Exception('Invalid font argument type')
    return Result
...
#                                                                                        #
#----------------------------------------------------------------------------------------#
class Assist_font(Toplevel):
    def __init__(self,Master=None,**kw):
        """
        The Font assist

        Our font specification is supposed to be one of three possibilities:
            None: indicating to use the default font
            A tuple: of (Family,Size, Modifiers)
            A string: of whatever the user wants, their issue to if it bombs
        """
        self.Font = ArgFetch(kw, 'ValueToEdit', Required=True)
        self.OptionName = ArgFetch(kw, 'OptionName', Required=True)
        self.Default = ArgFetch(kw, 'Default',Required=True)
        if self.Font == '':
            self.Font = None
        self.Initializing = 1

        apply(Toplevel.__init__,(self,Master),kw)
        self.bind('<KeyPress-Escape>',self.__on_ButtonCancel_ButRel_1)
        self.bind('<KeyPress-Return>',self.__on_ButtonOK_ButRel_1)
        self.__Frame13 = Frame(self)
        self.__Frame13.pack(anchor='w',side='top')
        self.__Label3 = Label(self.__Frame13,anchor='w',text='Font')
        self.__Label3.pack(anchor='w',padx=10,pady=10,side='top')
        self.__Frame5 = Frame(self,borderwidth=3,relief='raised')
        self.__Frame5.pack(anchor='w',fill='x',padx=5,pady=5,side='top')
        self.Mode = StringVar()
        self.__RadioDefault = Radiobutton(self.__Frame5,anchor='w'
            ,command=self.__on_RadioMode_command,value='Default',variable=self.Mode)
        self.__RadioDefault.pack(anchor='w',side='left')
        self.__LabelDefault = Label(self.__Frame5,text='Use default font.')
        self.__LabelDefault.pack(side='left')
        self.__Frame2 = Frame(self)
        self.__Frame2.pack(side='top')
        self.__Label1 = Label(self.__Frame2,text='--- OR ---')
        self.__Label1.pack(pady=4,side='top')
        ...
        self.__Label2 = Label(self.__Frame3,text='--- OR ---')
        self.__Label2.pack(pady=4,side='top')
        self.__Frame11 = Frame(self,borderwidth=3,relief='raised')
        self.__Frame11.pack(fill='x',padx=5,pady=5,side='top')
        self.__RadioUser = Radiobutton(self.__Frame11,command=self.__on_RadioMode_command
            ,value='User',variable=self.Mode)
        self.__RadioUser.pack(side='left')
        self.__LabelUser = Label(self.__Frame11,text='Use this code:')
        self.__LabelUser.pack(side='left')
        self.__EntryUser = Entry(self.__Frame11,width=45)
        self.__EntryUser.pack(padx=5,pady=5,side='left')
        self.__Frame1 = Frame(self)
        self.__Frame1.pack(fill='x',side='top')
        self.__ButtonOK = Button(self.__Frame1,text='Ok',width=7)
        self.__ButtonOK.pack(expand='yes',pady=10,side='left')
        self.__ButtonOK.bind('<ButtonRelease-1>',self.__on_ButtonOK_ButRel_1)
        self.__ButtonHelp = Button(self.__Frame1,text='Help',width=7)
        self.__ButtonHelp.pack(expand='yes',side='left')
        self.__ButtonHelp.bind('<ButtonRelease-1>',self.__on_ButtonHelp_ButRel_1)
        self.__ButtonCancel = Button(self.__Frame1,text='Cancel',width=7)
        self.__ButtonCancel.pack(expand='yes',side='right')
        self.__ButtonCancel.bind('<ButtonRelease-1>',self.__on_ButtonCancel_ButRel_1)
        self.__Frame6 = Frame(self.__Frame4)
        self.__Frame6.pack(side='left')
        self.__Frame8 = Frame(self.__Frame4,borderwidth=3,relief='raised')
        self.__Frame8.pack(expand='yes',fill='both',padx=5,pady=5,side='left')
        self.__Frame7 = Frame(self.__Frame4)
        self.__Frame7.pack(side='left')
        self.__Frame10 = Frame(self.__Frame8)
        self.__Frame10.pack(anchor='w',fill='both',side='top')
        self.__RadioFont = Radiobutton(self.__Frame10,anchor='w'
            ,command=self.__on_RadioMode_command,value='Font',variable=self.Mode)
        self.__RadioFont.pack(anchor='w',pady=5,side='left')
        self.__LabelFont = Label(self.__Frame10,text='Use this font:')
        self.__LabelFont.pack(side='left')
        self.__Frame12 = Frame(self.__Frame8)
        self.__Frame12.pack(expand='yes',fill='both',side='top')
        self.__Frame9 = Frame(self.__Frame8)
        self.__Frame9.pack(fill='x',side='top')
        self.Text1 = Text(self.__Frame9,height=6)
        self.Text1.pack(anchor='w',fill='x',side='top')
        self.__FrameLB = Frame(self.__Frame12)
        self.__FrameLB.pack(expand='yes',fill='both',side='left')
        self.__FrameAttr = Frame(self.__Frame12)
        self.__FrameAttr.pack(fill='both',padx=20,pady=10,side='left')
        self.LabelFilter = Label(self.__FrameAttr,text='Filter')
        self.LabelFilter.pack(side='top')
        self.__EntryFilter = Entry(self.__FrameAttr)
        self.__EntryFilter.pack(side='top')
        self.__EntryFilter.bind('<KeyRelease>',self.__on_EntryFilter_KeyRel)
        self.__Label4 = Label(self.__FrameAttr)
        self.__Label4.pack(side='top')
        self.FontSize = Label(self.__FrameAttr,text='Font size')
        self.FontSize.pack(side='top')
        #
        #Your code here
        #
        self.Filter = ''
        self.Searcher = Rph.Searcher()

        self.FamiliesListBox = Rph.ScrolledListbox(self.__FrameLB,label='Font Family',width=35)
        ...
        self.FamiliesListBox.FetchWidget().bind('<KeyRelease>',self.on_FamiliesKeyrelease)

        self.Counter1 = Rpw.Counter(self.__FrameAttr,entrywidth=5,modifiedcommand=self.on_UpdateNow)
        self.Counter1.pack(anchor='w',side='top')

        self.RadioSizeStyle = Rpw.RadioSelect(self.__FrameAttr,buttontype='radiobutton'
            ,command=self.on_UpdateNow,orient='vertical',pady=0)
        self.RadioSizeStyle.pack(anchor='w',ipadx=5,side='top')

        self.Label1 = Label(self.__FrameAttr)
        self.Label1.pack(side='top')

        self.RadioFontAttributes = Rpw.RadioSelect(self.__FrameAttr,buttontype='checkbutton'
            ,command=self.on_UpdateNow,orient='vertical',pady=0)
        ...
        #Set the radio buttons depending on the value to edit
        if self.Font == None:
            self.__RadioDefault.select()
            self.UserText = ''
        elif type(self.Font) == type(()):
            self.__RadioFont.select()
            self.UserText = ''
        else:
            self.__RadioUser.select()
            self.UserText = self.Font

        self.Initialize(FirstTime=True)
        self.Result = ''
    ...
    def __on_ButtonOK_ButRel_1(self,Event=None):
        ...
        if Mode == 'Default':
            ...
            if self.Font == None:
                Rpw.ErrorDialog('No font is selected.')
                return
            T = Validate_font(self.Font,self.OptionName)
            if T[0] <> 1:
                Rpw.ErrorDialog(T[0])
                return
            self.Result = self.Font
        elif Mode == 'User':
            T = self.__EntryUser.get()
            if len(T) == 0:
                Rpw.ErrorDialog('User code specification can not be blank.')
                return
            self.Result = T
        else:
            raise Exception
    ...
    def __on_EntryFilter_KeyRel(self,Event=None):
        """
        User released a key in the filter entry
        """
        self.Initialize()
        self.__EntryFilter.focus_set()

    def __on_RadioMode_command(self,Event=None):
        """
        User clicked on one of our Mode radio buttons
        """
        self.UserText = self.__EntryUser.get()
        self.Initialize()
    #
    #Start of non-Rapyd user code
    #
    def on_FamiliesKeyrelease(self,Event=None):
        """
        User pressed a key on the listbox.
        """
    ...
    def on_UpdateNow(self,Event=None,Extra=None):
        ...
        if self.Initializing:
            ...
            #initialized.
            return
        if 0: #We need the check the radio buttons here!!!
        ...
        #Tag ALL text so any the user may have entered gets rendered in the target font
        self.Text1.tag_add('demotext','1.0',END)
        self.Text1.tag_config('demotext',font=self.Font)

    def FilterFonts(self):
        """
        Filter and update the font list

        Result is the list of fonts.
        """
        self.Filter = self.__EntryFilter.get()
        Families = list(tkFont.families(self))
        Families.sort()
        #Populate the listbox with names of available fonts
        if self.Filter == '':
            #No filtering, show them all
            Fonts = Families
        else:
            #Apply the filter
            self.Searcher.Parse(self.__EntryFilter.get())
            Fonts = []
            for F in Families:
                if self.Searcher.Search(F):
                    Fonts.append(F)
        self.FamiliesListBox.setlist(Fonts)
        return Fonts

    def Initialize(self,FirstTime=False):
        """
        Initialize things depending on our Mode radiobuttons

        If FirstTime is true and the specified font is not available then we issue a
            message saying so.
        """
        self.Initializing = 1
        Md = self.Mode.get()
        #Enable them all so we can set stuff
        self.EnableWidgets('All')
        #Make the example text go away
        self.Text1.delete('1.0',END)
        if Md == 'Font':
            #We are not using the default font.
            #Populate the listbox with names of available fonts
            Families = self.FilterFonts()
            if len(Families) == 0:
                #Flag that we have no font selected.
                self.Font = None
            else:
                ...
                try:
                    ...
                    FontIndex = 0
                    if FirstTime:
                        Rpw.MessageDialog(Message='Unavailable font. Press Help for details'
                            ,Help=['font.unavailable',[self.Font[0]]])
            ...
            self.Text1.delete('1.0',END)
            self.EnableWidgets(Md)
            self.__EntryUser.delete(0,END)
            self.__EntryUser.insert(0,self.UserText)
        #Enable just the active ones
        self.EnableWidgets(Md)
    ...
    def EnableWidgets(self,Mode):
        """
        Enable/disable widgets based on mode, which must be Default, Font, User or All
        """
        D = F = U = DISABLED
        if Mode == 'Default':
            D = NORMAL
        ...
        self.__LabelDefault['state'] = D
        #The font selection area
        self.Counter1.Able(F==NORMAL)
        self.FontSize.config(state=F)
        self.FamiliesListBox.FetchLabel().config(state=F)
        for J in range(self.RadioSizeStyle.numbuttons()):
            self.RadioSizeStyle.button(J).config(state=F)
        for J in range(self.RadioFontAttributes.numbuttons()):
            self.RadioFontAttributes.button(J).config(state=F)
        self.__LabelFont['state'] = F
        self.LabelFilter['state'] = F
        self.__EntryFilter['state'] = F
        #The user entry area
        self.__LabelUser['state'] = U
        self.__EntryUser['state'] = U
        if Mode == 'User':
            self.__EntryUser.focus_set()
        elif Mode == 'Default':
            #We don't want the focus on the label, rather we went to get it away from
            #  UserEntry which would otherwise still be highlighted even through disabled.
            #  If there is a way to say "don't focus on anybody" it is unknown to me.
            self.__LabelDefault.focus_set()
        elif Mode == 'Font':
            self.__LabelFont.focus_set()
...
def Validate_float(Arg,OptionName):
    """
...
def Validate_command(Arg,OptionName):
    """
...
class Assist_command:
    ...
    def  __init__(self,ValueToEdit,CreateHandlerFlag,OptionName,HandlerList,NameOfWidreq):
        ...
        self.BB.add('Help',command=self.on_Help)
        self.BB.add('Cancel',command=self.on_Cancel)
        #Bind Enter and Escape to OK and Cancel
        self._Win.bind('<KeyPress-Return>',self.on_OK)
        self._Win.bind('<KeyPress-Escape>',self.on_Cancel)
        #be modal
        Rph.Grabber(self._Win)
        self._Win.wait_window()
    ...
    def on_OK(self,Event=None):
        ...
        if not Rpw.NameVet(NewValue,AllowDot=not self.CreateHandlerFlag):
            ...
            return
        Temp = Validate_command(NewValue,self.OptionName)
        if Temp[0] <> 1:
            Rpw.ErrorDialog(Temp[0])
            return
...
def Format_dim(Arg, Extra, Default):
    """
    Format dimensioned number for presentation to user
    """
    return Arg[2:]

def Validate_dim(Arg,OptionName):
    """
    ...
    Arg = str(Arg).strip()
    if len(Arg) < 2 or not Arg[:2] in ('n:','u:'):
       return ('Dimensioned number specification doesn\'t open with "n:" or "u:"',Arg)
    Prefix = Arg[:2]
    Detail = Arg[2:]
    if Prefix == 'n:':
        if len(Detail) == 0:
            return ('Blank is not a valid dimension value',Detail)
        try:
            int(Detail)
            return (1,Arg)
        except:
        ...
        if Detail[-1] in 'cimp':
            try:
                ...
                pass
        return ('"%s" is neither an integer nor a number followed by "c", "i", "m" or "p".'
            %Detail,Detail)
    else:
        if len(Detail) == 0:
            return ('User code secification can not be blank',Detail)
    return (1,Arg)

    #end Validate_dim

class Assist_dim(Toplevel):
    def __init__(self,Master=None,**kw):
        """
        The dimensioned number edit assist.
        ...
        Otherwise the result is a tuple:
            ...
            If they specified a dimensioned number then we have called the Validate_dim
            routine to make sure it is valid.
        ...
        apply(Toplevel.__init__,(self,Master),kw)
        self.bind('<KeyPress-Escape>',self.__on_Assist_dim_Key_Escape)
        self.bind('<KeyPress-Return>',self.__on_Assist_dim_Key_Return)
        self.__Frame5 = Frame(self)
        self.__Frame5.pack(anchor='w',side='top')
        self.__Label4 = Label(self.__Frame5,anchor='w',text='Dimensioned Number')
        self.__Label4.pack(anchor='w',padx=10,pady=10,side='top')
        self.__Frame3 = Frame(self,borderwidth=3,relief='raised')
        self.__Frame3.pack(fill='x',padx=10,pady=10,side='top')
        self.__Radiobutton1 = Radiobutton(self.__Frame3
    ...
    def __on_Assist_dim_Key_Escape(self,Event=None):
        """
        User pressed Escape
        """
        self.__on_ButtonCancel_ButRel_1(Event)

    def __on_Assist_dim_Key_Return(self,Event=None):
        """
        User pressed Return
        """
        self.__on_ButtonOK_ButRel_1(Event)
    ...
    def __on_ButtonHelp_ButRel_1(self,Event=None):
        ...
        User clicked on Help
        """
        Help('assist.dim',[self.OptionName,self.Default[2:]])
    ...
    def __on_ButtonOK_ButRel_1(self,Event=None):
        ...
        if self.SelectedButton.get() == 1:
            ...
            #They selected arb code
            NewValue = 'u:' +self.__EntryUser.get().strip()
        T = Validate_dim(NewValue,self.OptionName)
        if T[0] <> 1:
            #We don't like their specification
            Rpw.ErrorDialog(T[0])
            return
...
def Format_cvar(Text,Extra,Default):
    ...
    if not '.' in Extra:
        ...
        TypeInfo = ''
        if Extra[-2] == 'a':
            TypeInfo = {'i':',Int','s':',Str','f':',Float'}[Extra[-1]]
        Result = '%s [%s%s]'%(Text,{'a':'Auto','m':'Man'}[Extra[-2]],TypeInfo)
    return Result

def Validate_cvar(Arg,OptionName):
    """
...
class Assist_cvar:
    ...
    def  __init__(self, ValueToEdit, OptionName, ExtraInfo, WidreqName):
        ...
        self._Win = Toplevel()
        self._Win.title(Rph.Cap(OptionName))
        #The format of cvar ExtraInfo is described with Format_cvar
        ...
        self.AutoMan.add('Auto',text='Automatically create this control variable when the widget "%s" is created.'
            %WidreqName)
        self.AutoMan.add('Manual',text='I will create this control variable elsewhere.')
    ...
    def on_OK(self,Entry=None):
        NewValue = self.Entry.get()
        Temp = Validate_cvar(NewValue,self.OptionName)
        if Temp[0] <> 1:
            Rpw.ErrorDialog(Temp[0])
            return
...
def Validate_cursor(Arg,OptionName):
    """
...
class Assist_cursor:
    ...
    def on_OK(self,Event=None):
        NewValue = self.Entry.get()
        Temp = Validate_cursor(NewValue,self.OptionName)
        if Temp[0] <> 1:
            Rpw.ErrorDialog(Temp[0])
            return
        self.Result = str(Temp[1])
...
def Format_color(Arg, Extra, Default):
    """
    Format color for presentation to user
    """
    return Arg[2:]

def Validate_color(Arg,OptionName):
    """
    Verify that we like a color specification.
    ...
    The result is a tuple consisting of:
        o "[0]" 1 if we like the argument, or an error string if we don't.
        o "[1]" The argument, with possible modification by us. In particular
                if the user forgets the "#" we will supply it for them

    Note that an empty specification is not valid.
    """
    Arg = str(Arg).strip()
    if len(Arg) < 2 or not Arg[:2] in ('c:','v:'):
       return ('Color specification doesn\'t open with "c:" or "v:"',Arg)
    Prefix = Arg[:2]
    Detail = Arg[2:]
    if Prefix == 'c:':
        #Color specification
        if len(Detail) == 0:
            return ('Color specification can not be blank.',Detail)
        try:
            Button(bg=Detail)
        except:
            if Arg[:1] == '#':
                return ('"%s" is not an acceptable color specification.'%Detail,Detail)
            #perhaps they forgot the number-sign
            try:
                Button(bg='#'+Detail)
                Detail = '#' + Detail
            except:
                return ('"%s" is not a valid standard color.'%Detail,Detail)
    else:
        #Variable specification
        if len(Detail) == 0:
            return ('User code specification can not be blank',Detail)
    return (1,Prefix+Detail)

    #end Validate_color

class Assist_color(Toplevel):
    def __init__(self,Master=None,**kw):
        """
        The color edit assist.
        ...
        Otherwise the result is a tuple:
            [0] 'c' if they selected a color, 'v' if they selected arbitrary user code
            [1] The text of the color or variable

            If they selected a color then we have verified that it is either a valid color
            string (eg "#123456") or a valid color name (eg "red")

            If they selected user-code then no checks were done.
        ...
        self.bind('<KeyPress-Escape>',self.__on_ButtonCancel_ButRel_1)
        self.bind('<KeyPress-Return>',self.__on_ButtonOK_ButRel_1)
        self.__Frame7 = Frame(self)
        self.__Frame7.pack(anchor='w',side='top')
        self.__Label4 = Label(self.__Frame7,anchor='w',text='Color specification')
        self.__Label4.pack(anchor='w',padx=10,pady=10,side='top')
        self.__Frame3 = Frame(self,borderwidth=3,relief='raised')
        self.__Frame3.pack(fill='x',padx=10,pady=10,side='top')
        self.__Frame2 = Frame(self)
        ...
        self.__Label2 = Label(self.__Frame4,text='Use this code:')
        self.__Label2.pack(pady=20,side='left')
        self.__EntryVariable = Entry(self.__Frame4,width=35)
        self.__EntryVariable.pack(side='left')
        self.__EntryVariable.bind('<ButtonRelease-1>',self.__on_EntryVariable_ButRel_1)
        self.__Frame1 = Frame(self)
        self.__Frame1.pack(fill='x',side='top')
        self.__ButtonOK = Button(self.__Frame1,text='OK')
        self.__ButtonOK.pack(expand='yes',pady=20,side='left')
        ...
        self.__Frame6.pack(side='left')
        self.__Radiobutton1 = Radiobutton(self.__Frame6
            ,command=self.__on_Radiobutton1_command,value=1,variable=self.SelectedButton)
        self.__Radiobutton1.pack(side='left')
        self.__Label1 = Label(self.__Frame6,text='Use this color:')
        self.__Label1.pack(pady=20,side='left')
        self.__EntryColor = Entry(self.__Frame6)
        self.__EntryColor.pack(side='right')
        self.__EntryColor.bind('<ButtonRelease-1>',self.__on_EntryColor_ButRel_1)
        self.__Frame5 = Frame(self.__Frame3)
        self.__Frame5.pack(side='left')
        self.__ButtonChooser = Button(self.__Frame5,text='Color Chooser',width=13)
        self.__ButtonChooser.pack(padx=20,pady=5,side='top')
        self.__ButtonChooser.bind('<ButtonRelease-1>',self.__on_ButtonChooser_ButRel_1)
        self.__ButtonDefault = Button(self.__Frame5,text='Default',width=13)
        self.__ButtonDefault.pack(pady=5,side='top')
        self.__ButtonDefault.bind('<ButtonRelease-1>',self.__on_ButtonDefault_ButRel_1)
        #
        #Your code here
        #
        ...
        #Look after initial setup.
        if self.TypeFlag == 'c:':
            #We have a color specification
            self.__Radiobutton1.select()
            self.__EntryColor.insert(0,self.ValueToEdit)
            self.CurrentColor = self.ValueToEdit
            self.__EntryColor.focus_set()
        else:
            #We have a variable
            self.__Radiobutton2.select()
            self.__EntryVariable.insert(0,self.ValueToEdit)
            self.__CurrentColor = self.ValueToEdit
            self.__EntryVariable.focus_set()
    ...
    def __on_ButtonChooser_ButRel_1(self,Event=None):
        """
        User asked for color chooser
        """
        try:
            R = tkColorChooser.askcolor(color=self.__EntryColor.get())
        except TclError:
            #We didn't like their color. Try again with no color specified
            R = tkColorChooser.askcolor(color='#FFFFFF')
        if R == (None, None):
            #User cancelled
            return
        self.__Radiobutton1.select()
        self.__EntryColor.delete(0,END)
        self.__EntryColor.insert(0,R[1])
        self.CurrentColor = R[1]
        self.__EntryColor.focus_set()


    def __on_ButtonDefault_ButRel_1(self,Event=None):
        """
        User asked to revert to the default colors
        """
        self.__Radiobutton1.select()
        self.__EntryColor.delete(0,END)
        self.__EntryColor.insert(0,self.Default[2:])
        self.CurrentColor = self.Default[2:]
        self.__EntryColor.focus_set()
    ...
    def __on_ButtonHelp_ButRel_1(self,Event=None):
        ...
        User clicked on Help
        """
        Help('assist.color',[self.OptionName,self.Default[2:]])
    ...
    def __on_ButtonOK_ButRel_1(self,Event=None):
        ...
        if self.SelectedButton.get() == 1:
            ...
            NewValue = 'c:' +self.__EntryColor.get()
        else:
            #They selected a variable
            NewValue = 'v:' +self.__EntryVariable.get().strip()
        T = Validate_color(NewValue,self.OptionName)
        if T[0] <> 1:
            #We don't like their specification
            Rpw.ErrorDialog(T[0])
            return
    ...
    def __on_EntryColor_ButRel_1(self,Event=None):
        """
        User clicked on the color entry
        """
        self.__Radiobutton1.select()

    def __on_EntryVariable_ButRel_1(self,Event=None):
        """
        User clicked on the variable entry
        """
        self.__Radiobutton2.select()
    ...
    def __on_Radiobutton1_command(self,Event=None):
        """
        User clicked on the "use this color" radio button
        """
        self.__EntryColor.focus_set()

    def __on_Radiobutton2_command(self,Event=None):
        """
        User clicked on the "use this code" radio button
        """
        self.__EntryVariable.focus_set()
    #
    #Start of non-Rapyd user code
...
def Validate_bitmap(Arg,OptionName):
    """
...
class Assist_bitmap:
    ...
    def on_OK(self,Event=None):
        NewValue = self.Entry.get()
        Temp = Validate_bitmap(NewValue,self.OptionName)
        if Temp[0] <> 1:
            Rpw.ErrorDialog(Temp[0])
            return
        self.Result = str(Temp[1])
...
def Validate_bbox(Arg,OptionName):
    """
...
class Assist_bbox:
    ...
    def on_OK(self,Event=None):
        NewValue = self.Entry.get()
        Temp = Validate_bbox(NewValue,self.OptionName)
        if Temp[0] <> 1:
            Rpw.ErrorDialog(Temp[0])
            return
        self.Result = str(Temp[1])
...
class BindAssist:
    ...
    def  __init__(self,BindQuad, NameOfWidreq, ExistingEvents, HandlerList, ComponentList):
        ...
        for B in 'OK Help Cancel'.split():
            ...
            T.HelpTopic = 'General'
            T.bind(HelpButton,self.on_Help)
        #Bindings so we can use keys to get out
        self.Win.bind('<KeyPress-Return>',self.on_OK)
        self.Win.bind('<KeyPress-Escape>',self.on_Cancel)
        #be modal
    ...
    def on_Wizard(self):
        ...
        if R <> None:
            ...
            ##D('Name=<%s>'%Temp)
            #Create suggested handler name and put it in the HandlerCombo
            Prefix,Wn = WidgetNameSplit(self.NameOfWidreq)
            self.Handler = '%son_%s%s_%s'%(Prefix, Wn, Component,Temp)
            self.HandlerCombo.selectitem(self.Handler)
