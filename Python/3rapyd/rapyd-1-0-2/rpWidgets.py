...
def StringToFont(String):
    """
    Convert a font string to a font tuple.

    Font strings are of the form:

        <font family> <font-size> [<font-modifiers>]

    Where the modifies can be any of bold, italic, underline and overstrike.

    If we are passed a font-tuple, we will deal with it appropriately.

    If we can't decode the font we return an error string
    """
    if type(String) == type(()):
        #We got a tuple, turn it into a font-string
        String = FontToString(String)
    Tokens = String.split()
    Modifiers = []
    if len(Tokens) < 2:
        #We need at least <family> and <size>
        return 'Too few tokens to be a font string'
    while len(Tokens) > 0:
        Cur = Tokens.pop()
        if Cur in ('bold','italic','underline','overstrike'):
        ...
        try:
            ...
            return 'Expecting font size or modifier, but found "%s" instead'%Cur
    if len(Tokens) == 0:
        return 'Font family name not found'
    Family = ' '.join(Tokens)
...
class AAHash():
    """
    A simple hashing functing that will deal with characters above 127
    """
    def __init__(self):
        self._A = 0
        self._B = 0
    def update(self,Arg):
        for A in Arg:
            self._A = (self._A + ord(A)) & 0xFFFF
            self._B = self._B | ord(A)
    def digest(self):
...
class Counter(Frame):
    ...
    def __Sync(self):
        ...
        self.__Entry1.insert(0,strx(self.__CurrentCount))
...
class ColorizedText(Text):
    ...
    def __init__(self, Master=None, **kw):
        ...
        self.TextWidget.bind('<ButtonPress-1>',self.on_ButtonPress)
        ...
        #Usually our undo is enabled; occasionally we need to disable it
        self.UndoEnabled = True
        self.UndoStack = []
        self.RedoStack = []
        self.UndoMax = 1000
        self.OurSN = ColorizedText.SerialNumber
        ColorizedText.SerialNumber += 1
        ...
        """
        Undo/Redo. How they work and some arcane details. This will be of interest only if you
            are messing with the internal details of undo/redo; it is not of interest to users
            who simply want to use undo/redo.

            Our undo/redo is for the most part quite ordinary and uses a stack for undo and another
            for redo; nothing exciting here. Initially I tried the undo/redo built into the Tkinter
            Text but it wasn't even close to being up to the job so we wrote our own.

            Complication arises because some of the text is "SysText", that is, areas of text that
            are generated by and maintained by Rapyd-tk. Users are allowed to cursor through SysText
            but they are not allowed to make changes. The complication happens when we add to SysText,
            delete from SysText or move it around because all those operations can cause the users
            text, UserText, to change location which has implications at undo/redo time.

            The general idea for undo is to keep a stack of entries that say things like "At location
            34.17 this text was deleted". That is fine for text where the only changes are made by
            the user. As you undo your way through the stack things get put back as they were and
            everything is fine. In our case the changes SysText are not unodable and, if we use
            hard indicies like "34.17", things are not fine.

            The solution implemented here is to use a "base+offset representation" for UserText
            locations. We note that Rapyd-Tk forms are laid out thus:

                o SysText (class and def __init__)
                o First UserText area
                o SysText (apply and creation of widgets)
                o Second UserText area
                o SysText (#Start of event handler methods)
                o Zero or more event handlers consisting of
                  - SysText header ("def yadayada(self,")
                  - UserText body
                o SysText (#Start of non-Rapyd user code)
                o Last UserText area, users non-Rapyd code

            We assign a base of '0' through '6' for the areas prior to event handlers, we assign bases
            '-2' and '-1' to the two areas after event handlers while for event handler the base is the
            name of the handler. For handlers we need to know if the offset is from the start of the
            SysText or the UserText of the handler. For this we put a flag at the start of the offset.
            Flag values are:
                'x': Not a handler 
                's': SysText of a handler
                'u': UserText of a hander
            Thus, stored in the From and To entries of the undo/redo stack we see entries like this:

                0+x2.3
                1+x0.0
                2+x5.17
                on_Button1_Enter+u2.-31

            Where the characters before the '+' sign are the base ID and those after are the offset. Note that
            for event handlers negative offsets are common as the base starts part way through a line.

            When it comes time to undo/redo we convert these base+offset forms to hard Tkinter Text indices
            and it all works fine. Method UndoEncode converts to base+offset form while Method UndoDecode
            converts from base+offset form.

            As references to changes in UserText are all based on the start of the chunk of UserText it matters
            not if the absolute position of those chunks of UserText get pushed around or, in the case of
            event handlers, entirely relocated.

            If the user renames an event handler then we make a pass over the undo/redo stacks and make the
            name change there in any references to that handler. Thus changes in that handler can still be
            undone.

            If the user totally deletes an event handler (as opposed to just deleting the UserText portion of
            it) then we make a pass over the undo/redo stacks and make entries that refer to the deleted
            handler into "nil" entries which have no effect.

            Should you want to see the undo/redo stacks then turning on debugging "--debug" adds a text-edit
            Debug menu choice which has a "Dump the Undo stack" choice.
        """
    ...
    def CursorVisible(self):
        """
        If the cursor is not visible, place it mid text widget.
        """
    ...
    def Gather(self):
        ...
        for L in LineList:
            Hash.update(L)
        Result = {'Text':LineList,'Version':0,'Sha':Hash.digest()
            ,'Cursor':self.index(INSERT)}
        return Result

    def HandlerDeleteNotify(self,HandlerName):
        """
        Note that the event handler of the specified name has been deleted.

        We need this to properly handle undo.
        """
        #Go over the undo/redo stacks noting that the specified handler is gone
        self.UndoUpdate(HandlerName,None)

    def HandlerGoto(self,HandlerName):
        """
        Given a hander name, goto the first line of the handler
        """
        for Name, Start, End in self.HandlerNameList(Verbose=True):
            ##D('%s %s %s'%(Name,Start,End))
            if Name == HandlerName:
                #We've found our target
                self.TextWidget.mark_set(INSERT,Start+'+1l')
    ...
    def HandlerRenameNotify(self,Old,New):
        """
        Note that the event handler of the specified name has been renamed.

        We need this so we can undo edits in event handlers across renames.
        """
        #Go over the undo/redo stacks and modify any pointers that reference to old name to use the new name
        self.UndoUpdate(Old,New)
        self.HandlerGoto(New)

    def Redo(self):
        """
        Caller requests a Redo.

        This is pretty much the inverse of Undo.
        """
        RealEntriesProcessed = 0
        #Turn off undo so won't undo the undo.
        ...
        while 1:
            ...
            OrigType, From, To, TheText = X
            Type = OrigType[:3]
            if Type == 'sep':
                if RealEntriesProcessed > 0:
                    #??? Is this necessary for Redo?
                    #Done if we encounter a sep after a real entry, however, we want
                    #   to leave the separator on the stack so we move it from the
                    #   Undo stack back to the Redo stack.
                    #self.RedoStack.append(self.UndoStack.pop())
                    break
            elif Type == 'del':
                #Insert cut text
                ##DD('%s: Redo inserting cut text "%10s"'%(self.OurSN,TheText))
                UFrom = self.UndoDecode(From)
                if UFrom == None:
                    #This entry pertains to an event handler that is now gone
                    continue
                ##DD('    From Decoded %s --> %s'%(From,UFrom))
                self.insert(UFrom, TheText, tags='', UndoSeparator=False)
            elif Type == 'ins':
                #Delete inserted text
                ##DD('%s: Redo deleting inserted text "%10s"'%(self.OurSN,TheText))
                UFrom = self.UndoDecode(From)
                if UFrom == None:
                    #This entry pertains to an event handler that is now gone
                    continue
                #Compute To index given From and the actual text
                UTo = self.index('%s+%sc'%(UFrom,len(TheText)))
                ##DD('    From decoded %s --> %s'%(From,UFrom))
                ##DD('    To   computed as    %s'%(To,UTo))
                self.delete(UFrom,UTo,UndoSeparator=False)
            elif Type == 'cur':
                #Move the cursor as directed
                if To == None:
                    #This entry pertains to cursor motion from SysText. No action required
                    continue
                UTo = self.UndoDecode(To)
                if UTo == None:
                    #This entry pertains to an event handler that is now gone. No action is required.
                    continue
                if UTo == self.index('insert'):
                    #Moving the cursor to where it already is doesn't count as a real operation
                    continue
                self.mark_set('insert', UTo)
            elif Type == 'nil':
                pass
            else:
                raise Exception('Unknown undo Type "%s"'%Type)
            if Type in ('ins','del','cur'):
                RealEntriesProcessed += 1
        #Leave undo enabled
        self.UndoEnabled = True
        #In any case we want to be able to see the cursor.
        self.TextWidget.see(INSERT)
        return RealEntriesProcessed > 0
    ...
    def Scatter(self,Text):
        ...
        for T in Text['Text']:
            if len(T) > 0:
                #If we are inserting SysText we turn on PowerPill; the undo feature cares
                self.insert(END,T,Tag,PowerPill=Tag=='bgldt')
            if Tag == 'bgldt':
                Tag = ''
            else:
                Tag = 'bgldt'
        self.mark_set(INSERT,self.IndexToLeft(Text['Cursor']))
        #We use insert-1l here because Text.see() thinks that your are seeing a line event
        #  if what you are seeing is only the bottom pixel-row of the line. Asking to see
        #  the previous line makes sure you can see the full line.
        self.see('insert-1l')
        #Clear our undo stack
        ##DD('%s: Undo stack cleared! -----------------------------------'%self.OurSN)
        self.UndoStack = []
        self.RedoStack = []
    ...
    def SystextReplace(self,ID,NewText):
        ...
        #Insert the new text
        self.insert(Ranges[I],NewText,'bgldt',PowerPill=1)

    def Undo(self):
        """
        Caller requests an undo.
        
        There are three kinds of entries on the undo stack:
        
            Real entries. These are things that need to be undone: insert, delete and cursor motion.
            
            Separator entires. These tell us to stop the undo operation.
            
            Advice entries. These tell us about rename or delete of event handlers.

        Any one undo operation will run to end-of-stack or until we encounter a separator. However, 
            assuming we haven't run out of stack, we want to process at least one real entry before
            stopping. Therefore we keep a running count of the number of real entries that we have
            processed. If we hit a separator AND we have processed zero real entries then we ignore
            the separator and carry on. 

        Result is True if we actually undid something, else False
        """
        RealEntriesProcessed = 0
        #Turn off undo so won't undo the undo.
        self.UndoEnabled = False
        ...
        while 1:
            ...
            if Type == 'sep':
                if RealEntriesProcessed > 0:
                    #Done if we encounter a sep after a real entry, however, we want
                    #   to leave the separator on the stack so we move it from the
                    #   Redo stack back to the Undo stack.
                    self.UndoStack.append(self.RedoStack.pop())
                    break
            elif Type == 'ins':
                #Insert cut text
                ##DD('%s: Undo inserting cut text "%10s"'%(self.OurSN,TheText))
                UFrom = self.UndoDecode(From)
                if UFrom == None:
                    #This entry pertains to an event handler that is now gone
                    continue
                ##DD('    From decodeed %s --> %s'%(From,UFrom))
                self.insert(UFrom, TheText, tags='', UndoSeparator=False)
                self.mark_set('insert',UFrom)
            elif Type == 'del':
                #Delete inserted text
                ##DD('%s: Undo deleting inserted text "%10s"'%(self.OurSN,TheText))
                UFrom = self.UndoDecode(From)
                if UFrom == None:
                    #This entry pertains to an event handler that is now gone
                    continue
                #Compute To extent given From and the actual text
                UTo = self.index('%s+%sc'%(UFrom,len(TheText)))
                ##DD('    From decoded %s --> %s'%(From,UFrom))
                ##DD('    To   computed as    %s'%(To,UTo))
                self.delete(UFrom,UTo,UndoSeparator=False)
                ...
                if To == None:
                    #Save our current position on the redo stack so we know where to redo to
                    To = self.index('insert')
                    To = self.UndoEncode(To)
                    #Note that if we are currently inside SysText then UndoEncode will return None
                    #  which we will store. Redo knows to bypass such entries.
                    self.RedoStack[-1] = ((OrigType,From,To,TheText))
                #Move the cursor as directed
                UFrom = self.UndoDecode(From)
                if UFrom == None:
                    #This entry pertains to an event handler that is now gone
                    continue
                if UFrom == self.index('insert'):
                    #Moving the cursor to where it already is doesn't count as a real operation
                    continue
                self.mark_set('insert', UFrom)
                ##DD('    Setting cursor to %s'%UFrom)
            elif Type == 'nil':
                #Nil entries were originally references to changes in an event handler that is now deleted.
                #  No action is necessary and it doesn't count as a real entry processed.
                pass
            else:
                raise Exception('Unknown undo Type "%s"'%Type)
            if Type in ('ins','del','cur'):
    ...
    def UndoSeparator(self):
        """
        Push a separator on the undo stack.

        As we automatically put separators on the stack most of the time so this
            is rarely needed. However, if you have done some inserts/deletes
            while specifically requesting no separator then this allows you
            to insert a separator following the operation
        """
        self.UndoPush('sep.UndoSeparator()')
    ...
    def delete(self, index1, index2=None, UndoSeparator=1, PowerPill=0):
        """
        Don't allow deletion of look-don't-touch systext unless "PowerPill" is true

        If PowerPill is True then we disable the undo feature for this delete.

        If PowerPill is False then by default we push a separator on the undo
            stack; set UndoSeparator to False to disable this behavior.
        """
        ...
        if not PowerPill:
            ...
            if len(Next) <> 0:
                return
        #Create an undo entry if appropriate
        if self.UndoEnabled:
            TheTextBeingDeleted = self.TextWidget.get(index1,index2)
            if PowerPill:
                #We are deleting systext
                pass
            else:
                #This is to put the cursor back where it was
                self.UndoPush('cur.delete')
                #This undoes the delete
                self.UndoPush(('ins.delete',From,None,TheTextBeingDeleted))
        #And not the actual delete
        Text.delete(self,index1,index2)
        if self.UndoEnabled and UndoSeparator and not PowerPill:
            #Undo is enabled, we want a separator and were not doing SysText
            self.UndoPush('sep.delete')


    def insert(self, index, text, tags=None, ProgressFunction=None, UndoSeparator=1, PowerPill=0):
        """
        ...
        Set PowerPill True if inserting systext. This will prompt us to turn
            undo off before doing the insert so the user can't undo it.

        We expand tabs from the source file before doing the insert.
        """
        From = self.index(index)
        InsertPoint = int(float(From))
        ExText = text.expandtabs()
        Text.insert(self,index,ExText,tags)
        if self.UndoEnabled:
            if PowerPill:
                #We are inserting systext
                pass
            else:
                #To undo we need to delete from the insert-cursor to insert-cursor plus length of inserted text
                self.UndoPush(('del.insert',From,self.index('%s+%sc'%(From,len(ExText))),ExText))
        Count = text.count('\n')
    ...
    def mark_set(self,TheMark, TheIndex, Undo=True, UndoSeparator=True):
        """
        We need this one so we can save undo info.

        Caller can turn off the entire Undo save or just the Undo Separator
        """
        if self.UndoEnabled and Undo:
            if TheMark == 'insert':
                #Moving the insert mark is implicit cursor motion
                self.UndoPush('cur.mark_set')
                if UndoSeparator:
                    #Add a separator unless we were told not to
                    self.UndoPush('sep.mark_set')
        Text.mark_set(self,TheMark,TheIndex)
    ...
    def on_ButtonPress(self,Event):
        ...
        """
        #Create an entry so we can undo this
        self.UndoPush('cur.on_ButtonPress')
        self.UndoPush('sep.on_ButtonPress')
    ...
    def on_KeyPress(self,Event):
        ...
        if Event.keysym in ('Delete','BackSpace'):
            ...
            if self.SystextBookendsRegion(From,To):
                ...
                self.DeletionFromTo = None
                return 'break'
            #We have gone to some trouble to develop the From and To of the delete on behalf of
            #  the colorizer but it just what we need for the undo stack as well.
            if self.UndoEnabled:
                self.UndoPush(('ins.on_KeyPress_0',From,None,self.TextWidget.get(From,To)))
                self.UndoPush('sep.on_KeyPress_0')
            ...
            if len(Event.char) == 1 and Event.char >= ' ' and Event.char <= '~':
                ...
                #By inserting it ourselves the character gets colorized with no visible delay.
                self.insert(INSERT,Event.char)
                return 'break'
            if Event.keysym == 'Return':
                #Create undo-stack entry for Enter
                From = self.index('insert')
                self.UndoPush(('del.on_Keypress',From,From+'+1c','\n'))
            #We have a non-graphic character
            ...
            if Event.keysym in ('Up','Down','Left','Right','Home','End'):
                #Create undo-stack entry for motion keys
                self.UndoPush('cur.on_KeyPress_2')
                self.UndoPush('sep.on_KeyPress_2')
    ...
    def IndexToTuple(self,Index):
        """
        Convent an index to a 2-tuple of integers

        For example given an index of "29.17" we return (28,17).
        """
        T = Index.split('.')
        return (int(T[0]),int(T[1]))

    def TupleToIndex(self,Tup):
        """
        Given a 2-tuple convert it to an Text index.

        This is the invert of IndexToTuple
        """
        return '%s.%s'%Tup
    ...
    def ColorizeInBackground(self,LineNumber):
        ...
        if self.CibState == self.CibSt.Running:
            ...
            ##D('EndRegroup. NextLine=%s Range=%s'%(NextLine,strx(Range)))
        #
        # Wrap up and go home
        #
    ...
    def LineColorize(self,Line,Flag=0):
        ...
        for Tag in TagList:
            ...
            NewColorStateTag = 'ColorFlag%s'%Flag
            self.TextWidget.tag_add(NewColorStateTag,ColorTagIndex)
        #Mark the line colored by putting a "C" tag on the entire line. It is important that it cover
        #  the entire line because CIB looks for as-yet uncolorized text by getting the "C" ranges.
    ...
    def UndoDump(self):
        ...
        Width = 80
        print
        print 'Undo Stack'.center(Width,'-')
        Indicies = range(len(self.UndoStack))
        Indicies.reverse()
        for I in Indicies:
            Type, From, To, Text = self.UndoStack[I]
            if type(Text) == type (''):
                Text = Text.replace('\n','|')
                if len(Text) > 80:
                    Text = Text[:80] + '<snip>'
                Text = '"%s"'%Text
            print '    %s %s %s %s'%(Type, From, To, Text)

        print 'Redo Stack'.center(Width,'-')
        Indicies = range(len(self.RedoStack))
        Indicies.reverse()
        for I in Indicies:
            Type, From, To, Text = self.RedoStack[I]
            if type(Text) == type (''):
                Text = Text.replace('\n','|')
                if len(Text) > 80:
                    Text = Text[:80] + '<snip>'
    ...
    def UndoDecodeOld(self,BaseOffsetIndex):
        """
        Decode a Base+Offset index to an absolute index.

        Result is an index-string or None if the index referenced text in an event handler
            which has since been deleted.
        """
        Base, Offset = BaseOffsetIndex.split('+')
        Row, Col = self.IndexToTuple(Offset)
        Ranges = self.tag_ranges('bgldt')
        ##print 'Decode (%s,%s  :  %s,%s), %s'%(Base,Offset, Row,Col,Ranges)
        try:
            #If the base is numeric is specifies a particular piece of UserText
            Base = int(Base)
            if Base == -1:
                #Offset is from the last usertext
                if len(Ranges) == 0:
                    #If we have no SysText then be default we originate at start-of-text
                    FromIndex = "1.0"
                else:
                    #We originate from the last range
                    FromIndex = Ranges[-1]
            elif Base == 0:
                #Offset is from the start of text
                FromIndex = "1.0"
            else:
                #Offset is from a specified usertext
                FromIndex = Ranges[(Base*2)-1]
        except ValueError:
            #The base isn't numeric, it's the name of an event handler.
            #Ranges[6] is the index of the first event handler
            I = 6
            while 1:
                if I >= len(Ranges):
                    #We are out of SysText without finding the 
                    return None
                HIndex = self.IndexToTuple(Ranges[I+1])
                #HIndex points to the end of the handler header
                HandlerName = self.get(self.TupleToIndex((HIndex[0],8)),self.TupleToIndex((HIndex[0],HIndex[1]-5)))
                if HandlerName == Base:
                    #We have our match
                    FromIndex = Ranges[I+1]
                    break
                #Not found next, advance to next handler
                I += 2
        #From is the Text index to which we apply the offset
        FromRow, FromCol = self.IndexToTuple(FromIndex)
        ##print 'From = (%s,%s)'%(FromRow,FromCol)
        Result = self.TupleToIndex((FromRow+Row, FromCol+Col))
        ##print 'Decode Result="%s"'%Result
        return Result

    def UndoDecode(self,BaseOffsetIndex):
        """
        Decode a Base+Offset index to an absolute index.
        ...
        try:
            ...
            if Base == -1:
                #Offset is from the last UserText
                if len(Ranges) == 0:
                    #If we have no SysText then by default we originate at start-of-text
                    FromIndex = "1.0"
                else:
                    #We originate from the last range
                    FromIndex = Ranges[-1]
            elif Base == -2:
                #Offset is from last SysText
                FromIndex = Ranges[-2]
            elif Base == 0:
                #Offset is from the start of text
                FromIndex = "1.0"
            else:
                #Offset is from a specified SysText/UserText prior to the event handlers
                FromIndex = Ranges[Base-1]
        except ValueError:
            #The base isn't numeric, it's the name of an event handler.
            #Ranges[6] is the index of the first event handler
            I = 6
            while 1:
                ...
                if HandlerName == Base:
                    #We have our match
                    if Flag == 's':
                        #Offset is from the SysText of the handler
                        FromIndex = Ranges[I]
                    else:
                        #Offset is from the UserText of the handler
                        FromIndex = Ranges[I+1]
                    break
                #Not found next, advance to next handler
                I += 2
    ...
    def UndoEncode(self,HardIndex):
        """
        Encode an absolute text index ("23.7") into a base+offset from ("0+x3.7").

        Result with be the base+offset index or None.
        """
        HardIndexF = self.IndexToTuple(HardIndex)
        Ranges = self.tag_ranges('bgldt')
        #The ranges are index-pairs saying "SysText runs from X to Y"
        ##print '%s %s %s %s'%(self.OurSN,HardIndex,HardIndexF,Ranges)
        I = 0
        BaseIndex = (0,0)
        BaseName = 0
        while 1:
            if I >= len(Ranges):
                ...
                BaseName = -1
                break
            RangeF = self.IndexToTuple(Ranges[I])
            if HardIndexF <= RangeF:
                #HardIndexF is prior to the start of the next text area, therefore it
                #  is in the area just prior. We already have the index of the
                #  start the prior area in BaseIndex we we are done
                break
            BaseIndex = RangeF #Note index of start text area
            BaseName += 1
            I += 1
        if BaseName > 4 and BaseName == len(Ranges)-1:
            #We are in the final SysText area
            BaseName = -2
        #We have the BaseIndex and the BaseName
        if BaseName > 6:
            #This is an event handler. Get text to use as base nmae.
            #At this point an odd BaseName means we are in the SysText header of the handler, while an
            #  even number means we are in the UserText body of the handler.
            if (BaseName & 1) <> 0:
                #Odd; We are in SysText, get the index of the following usertext to use when getting the name
                NameIndex = self.IndexToTuple(Ranges[I])
                Flag = 's'
            else:
                #Even; We are in UserText, BaseIndex is our NameIndex
                NameIndex = BaseIndex
                Flag = 'u'
            #Extract the name of the handler
            BaseName = self.get(self.TupleToIndex((NameIndex[0],8)),self.TupleToIndex((NameIndex[0],NameIndex[1]-5)))
        else:
            #Flag that this is not an event handler
            Flag = 'x'
        #We have our original hard index as a tuple and we have our base index as a tuple; compute the offset
        BaseRow, BaseCol = BaseIndex
        HardRow, HardCol = HardIndexF
        OffRow = HardRow - BaseRow
        OffCol = HardCol - BaseCol
        Result = '%s+%s%s.%s'%(BaseName,Flag,OffRow,OffCol)
        return Result

    def UndoEncodeOld(self,HardIndex):
        """
        Encode an absolute text index ("23.7") into a base+offset from ("0+3.7").

        Result with be the base+offset index or None if the HardIndex was inside SysText.
        """
        EByNew = self.UndoEncodeNew(HardIndex)
        #D('%s: NewE="%s"'%(self.OurSN,EByNew))
        #D(  '  NewD="%s" Org="%s"'%(self.UndoDecodeNew(EByNew),HardIndex))
        ...
        while 1:
            if I >= len(Ranges):
                ...
                BaseName = -1
                break
            SysFrom = self.IndexToTuple(Ranges[I])
            SysTo = self.IndexToTuple(Ranges[I+1])
            if HardIndexF > SysFrom and HardIndexF < SysTo:
                #Agg! It's in SysText
                ##print 'In SysText, returning None'
                return None
            if HardIndexF <= SysFrom:
                #HardIndexF is prior to the start of the next SysText area, therefore it
                #  is in the UserText area just prior. We already have the index of the
                #  start the prior UserText area in BaseIndex
                break
            BaseIndex = SysTo #Note index of start of UserText area, which follows the SysText
            BaseName += 1
            I += 2
        #We have the BaseIndex and the BaseName
        if BaseName > 3:
            #This is an event handler. Get text to use as base nmae
            BaseName = self.get(self.TupleToIndex((BaseIndex[0],8)),self.TupleToIndex((BaseIndex[0],BaseIndex[1]-5)))
        #We have our original hard index as a tuple and we have our base index as a tuple; compute the offset
        BaseRow, BaseCol = BaseIndex
        HardRow, HardCol = HardIndexF
        OffRow = HardRow - BaseRow
        OffCol = HardCol - BaseCol
        Result = '%s+%s.%s'%(BaseName,OffRow,OffCol)
        return Result

    def UndoPush(self,Arg):
        """
        Add an entry to the undo stack.

        Nominally arguments are a 4-tuple of:
            [0] Type. A string, see below.
            [1] A start index or None.
            [2] An end index or None.
            [3] Text associated with this entry, or None

        Valid types:
            'cur': Note a command that moves the cursor.
            'del': Note that a delete is required to undo the operation. From and To must be supplied.
            'ins': Note that an insert is required to undo the operation. From and Text must be supplied.
            'sep': Add a separator to the undo stack

            You can place commentary text after the valid type to indicate who posted the request.
            If Cfg.Info['Debug'] is true we retain this information for debugging purposes.

        A number of types consist solely of a type string and nothing else.
            if Arg is of type string then we take it to be 
            (<string>,None,None,None)

        A push on the undo stack implicitly clears the redo stack.
        """
        #Overall sanity checks
        assert self.UndoEnabled
        if type(Arg) == type(''):
            Arg = (Arg,None,None,None)
        assert type(Arg) == type(())
        assert len(Arg) == 4
        Type, From, To, Text = Arg
        assert type(Type) == type('')
        assert type(Text) in (type(''), type(None), type(u''))
        Comment = Type[3:]
        Type = Type[:3]
        assert Type in ('cur','del','ins','sep')
        #Normalize From and To
        if From <> None:
            From = self.index(From)
        if To <> None:
            To = self.index(To)
        UFrom = From
        UTo = To
        ...
        if Type == 'sep':
            if self.UndoStack <> [] and self.UndoStack[-1][0][:3] == 'sep':
                #Top stack entry is already a sep; nothing to do
                return
        elif Type == 'ins':
            assert From <> None
            assert To == None
            assert type(Text) in (type(''),type(u''))
            UFrom = self.UndoEncode(From)
        elif Type == 'del':
            assert From <> None
            assert To <> None
            assert type(Text) in (type(''),type(u''))
            UFrom = self.UndoEncode(From)
            UTo = self.UndoEncode(To)
        elif Type == 'cur':
            assert From == None
            assert To == None
            assert Text == None
            #Save the position of the insert cursor so undo can put it back
            UFrom = self.index('insert')
            UFrom = self.UndoEncode(UFrom)
            ##DD('   UFrom encoded to %s'%UFrom)
        #Now do the actual work
        if Cfg.Info['Debug']:
            #If were debugging then retain the commentary
            Type = Type + Comment
        self.UndoStack.append((Type,UFrom,UTo,Text))
        ##DD('%s: Undo stack push: %s %s %s %10s'%(self.OurSN,Type,From,To,Text))
        ##DD('    From encoded %s ---> %s'%(From,UFrom))
        ##DD('    To   encoded %s ---> %s'%(To,UTo))
        if len(self.UndoStack) > self.UndoMax:
            self.UndoStack.pop(0)
        self.RedoStack = []

    def UndoUpdate(self,Target,New):
        """
        Scan the Undo and Redo stacks, make possible updates

        Target will be the name of an event handler.

        New will be either the name of an event handler or None.

        If New is the name of an event hander then and From/To entries in the stacks that
            are based on handler Target are revised to be based on handler New.

        If New is None then any entry in the stacks whose From or To entries is based on
            Target is converted to a "nil" entry as the handler has been deleted.
        """
        for TheStack in (self.UndoStack, self.RedoStack):
            for J in range(len(TheStack)):
                #Scan the stack looking for entries the need to be updated
                Type, From, To, TheText = TheStack[J]
                ##print 'UndoUpdate: %s'%strx(TheStack[J])
                if From == None:
                    FromBase = None
                else:
                    FromBase, FromOffset = From.split('+')
                if To == None:
                    ToBase = None
                else:
                    ToBase, ToOffset = To.split('+')
                UpdateEntry = False
                if New == None:
                    #We are noting a deleted handler
                    if FromBase == Target or ToBase == Target:
                        Type = 'nil'
                        From = None
                        To = None
                        UpdateEntry = True
                else:
                    #We are noting a renamed handler
                    if FromBase == Target:
                        FromBase = New
                        UpdateEntry = True
                    if ToBase == Target:
                    ...
                    if UpdateEntry:
                        if From <> None:
                            From = '%s+%s'%(FromBase,FromOffset)
                        if To <> None:
                            To = '%s+%s'%(ToBase,ToOffset)
                if UpdateEntry:
                    TheStack[J] = (Type,From,To,TheText)
...
class NewProjectDialog(ToplevelRP):
    ...
    def on_OK(self,Event=None):
        ...
        if not NameVet(Temp[0]):
            #Their name isn't Python-legal
            MessageDialog(Title='Error', Message='Sorry, "%s" is not an acceptable name for a project.'%Temp[0]
                ,Help='project.new.dialog.badname')
            return
        FileList = []
        Path = self.ProjectDir + os.path.sep + Temp[0]    
        Filename = Path+'.py'
        ...
        if len(FileList) <> 0:
            ...
            if R <> 1:
                return
        self.Result = (self.ProjectDir + os.path.sep, Temp[0])
        self.destroy()
...
def NameVet(Name,AllowDot=False):
    ...
    If AllowDot is True then we return True if Name is one or more
        Python-legal names separated by dots.

    Result is True if we like the name, else Valse.
    """
    if AllowDot:
        NameList = Name.split('.')
    else:
...
class PromptDialog:
    ...
    def  __init__(self,Title='Query',Message='Message',Help=None,Prompt='',XY=None):
        ...
        self.LabelStringVar.set(Rph.TextBreak(strx(Message),34))    
        #
        # Entry area
        #
...
class MessageDialog:
    ...
    If "Buttons" is omitted you get a single 'Dismiss' button. Otherwise "Buttons" should 
        ...
        Usually there is a 'contrary' button which is the opposite of the 'affirmative'
            ...
            is removed before the button is created.
    ...
    def  __init__(self,Title='Information'
        ...
        self.LabelStringVar.set(Rph.TextBreak(strx(Message),44))    
        HelpPlaced = False
        if Modal:
            ...
            if License: 
                ...
                self.Label2StringVar.set(Rph.TextBreak(strx(Message),44))    
...
class HandlerRenameDialog(ToplevelRP):
    ...
    RefCount: The number of Rapyd maintained references to this handler.

    Result is new, valid, non-conflicting name chosen by the user or None if they cancel.
    """
    def __init__(self, Master=None, **kw):
        self.HandlerName = kw['HandlerName']
        del kw['HandlerName']
        ...
        if self.RefCount == 0:
            Msg = ('No Rapyd-Tk maintained bindings nor command-options refer to this event handler.\n'
                'Changing the event handler name will not affect any Rapyd-Tk maintained bindings or command-options.')
        else:
...
if __name__ == '__main__':
    ...
    for J in (
        ...
        'bold',
        '12',
        'times roman 12 bold italic underline overstrike',
        'courier 12 zotr',
        ):
        print J
        print StringToFont(J)
        print
