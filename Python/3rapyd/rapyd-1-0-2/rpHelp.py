...
def GeometryEncode(GeometrySequence,Full=False):
    """=u
    Given a tuli of four numbers, return a geometry string.

    If Full is True you get back the full string:  wxh+x+y

    If Full is not True you get the the offset form: +x+y
    """
    if Full:
        Result = '%dx%d%+d%+d'%tuple(GeometrySequence)
    else:
        Result = '+%d+%d'%tuple(GeometrySequence[-2:])
    return Result
...
class TheHelpThingy:
    ...
    This class provides user help.
     ...
     o "BasicFontSize" The size of the font used for everything except headings.

     o "HeadingFontSize" The size of the font used for headings.

     o "WindowSize" The size of the initial help window as a percentage of the size
        of the screen. Defaults to 60.
    ...
    def __init__(self
        ...
        assert self.WindowSize >= 0 and self.WindowSize <= 100,self.WindowSize
        self.WindowSize = self.WindowSize / 100.0
    ...
    def DisplayKnown(self, HelpTopicID, PassedArgumentList=None):
        ...
        *Help, grab and modal dialogs.* 
        ...
        if self.Win == None:
            ...
            self.Win.bind('<KeyPress-Escape>',self.on_Close)
            ScreenWidth = self.Win.winfo_screenwidth()
            Wid = max(int(round(ScreenWidth * self.WindowSize)),200)
            ScreenHeight = self.Win.winfo_screenheight()
            Hgt = max(int(round(ScreenHeight * self.WindowSize)),200)
            if self.Locate:
                ...
                self.Win.geometry('%sx%s+%s+%s'%(Wid,Hgt,Xpos,Ypos))
            else:
                self.Win.geometry('%sx%s+50+50'%(Wid,Hgt))
            #
        ...
        if HelpTopicID[0] == '#':
            ...
            if TheText == '' or TheText[0] <> '>':
                ...
                #This is experimental. If the passed argument list is longer than the argument list supplied
                #by the redirect line, then we use as many as we got from the redirect line and after that
                #use the ones from the passed list.
                #The us the passed list was 1 2 3 4 and the redirect line supplied 55, 56
                #The effective argument list would be 55, 56, 3, 4
                if PassedArgumentList <> None:
                    ArgumentList = ArgumentList+PassedArgumentList[len(ArgumentList):]
    ...
    def on_Destroy(self,Event):
        ...
        For reasons that I don't begin to comprehend this routine gets
            called about 16 times when the user closes help. ?????
        """
